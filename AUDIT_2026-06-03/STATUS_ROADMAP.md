...
**✔ AUTON-01 SHADOW ZBUDOWANY 2026-06-13 (nocna sesja, commit `a7efd21` tag `auton01-shadow-2026-06-13`, PUSHED)** — projekt `eod_drafts/2026-06-13/AUTON01_DESIGN.md`: bramka `auto_assign_gate.evaluate_auto_assign` (G1-G11: klasyfikator F7 AUTO na marginie Z-10 + pos informed/nie-store + bez late-pickup/R6/commit-div + wyłączenia czasówki/paczki/rampa/scarcity + sufit nieufności score>90 wg Bartek 2.0 §4.1), telemetria `would_auto_assign`+`auto_block_reasons[]` compute-zawsze (lekcja #186) serializowana top-level w shadow_decisions; egzekutor `auto_assign_executor` ZA FLAGĄ `ENABLE_AUTO_ASSIGN` (kanon ETAP4 flags.json=**false**, hot killswitch; OFF=zero pracy) — rate-cap 6/h + cooldown 60 min po PANEL_OVERRIDE + wykonanie subprocess gastro_assign (ścieżka ASSIGN_DIRECT) + TG post-hoc przez telegram_utils (dispatch-telegram NIETKNIĘTY). 71 testów, suita=baseline (A/B stash-diff pusty). **Aktywacja telemetrii = restart shadow at#139 02:30 UTC 13.06** (bramka pre-restart zielona). ⚠ Realny assign NIGDY nie przeszedł E2E — flip po E7 za ACK + osobne E2E z Adrianem. **Kalibracja (analiza siostrzanej sesji `AUTON01_ACCEPTANCE_SEGMENTS.md`, n=245): bramki flipu „≥200 would_auto / ≥75% acceptance" dziś nieosiągalne (pełny stos → n≈1; AUTO=27% acceptance=średnia; margin NIE przewiduje acceptance; gold=16% najgorszy tier) → E7 (at#131 17.06) kalibruje na ROZKŁADZIE auto_block_reasons + acceptance-first targeting (std/std+ × off-peak), anomalia gold do wyjaśnienia przed flipem; G7 pos-informed przy dzisiejszej adopcji GPS (1 nadawca) = de facto wyłącznik → adopcja apki to prerekwizyt autonomii (GPS_ADOPTION_DIAGNOSIS.md).**
Zostaje: AUTON-02 (4% AUTO), AUTON-04 (próg C2 placeholder — częściowo skonsumowany przez progi T1/T2/T3 + stałe AUTO_ASSIGN_*), AUTON-08 (batching/continuous re-opt), AUTON-09 (wyuczony model ETA). A2 flipnięty live (coeff 100, 11.06).
