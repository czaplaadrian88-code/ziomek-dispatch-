...
> **Aktualizacja 2026-07-07 (historyczny baseline guard; nie semantyka ODR-001):** dozbrojono trzy seamy bez zmiany silnika: `test_inv_r6_dial_family` pilnuje spójności starych diali 35, `test_inv_lexqual_geometry_group_subordination` subordynacji geometrii i `test_inv_carried_first_lock_first` lock-first. Wszystkie miały mutation probe. Od 12.07 pierwszy test dowodzi tylko parytetu baseline, nie poprawności eventu possession→handoff ani trybu Alarm; aktualny status opisuje INV-FEAS-R6-ONE-SOURCE niżej. Raport: `eod_drafts/2026-07-07/B2_inwarianty.md`.
> **Ustalenie uboczne 07.07 zachowane:** dashboard miał STALE sloty uzbrojone wcześniej przez L7.x/L6.C: INV-LAYER-HARD-BEFORE-SOFT i INV-LAYER-NO-VERDICT-OUTSIDE-L5 (`test_split_layer_guard_l73`) oraz INV-COH-R-DECLARED (`test_r_declared_tripwire_l71`). Węższe rezydua są opisane przy właściwych inwariantach niżej.

> **Aktualizacja 2026-07-08 (Sprint B bezpieczniki alokacji — zakres NIE-ETA; ZERO flipów/flags.json):** uzbrojono pusty SLOT **INV-FEAS-NO-DOUBLE-BOOK 🔴→🟢** (Kontrakt ②, l.36) — strażnik spójności claim-ledger (log-loud tripwire, twarda blokada odłożona za ACK). Weryfikator `claim_ledger.verify_no_stale_claim` (leaf, pure) wpięty w OBA bliźniaki RAZEM (`global_allocate` resweep + `_tick` shadow za ENGINE_CLAIM_LEDGER); 2 flagi decyzyjne `ENABLE_CLAIM_LEDGER_INVARIANT_{CHECK,HARD}` (ETAP4, default OFF, NIE w flags.json); metryka `g_claim_ledger_breaches` w jsonl+summary. 18 testów + mutation-probe (neutralizacja `tentative_assign` → pile-on wykryty) + **zero-FP DOWÓD: 5000 losowych sweepów, 3092 z legalnym bundlingiem = 0 fałszywek**; regresja pełna zielona. Sloty ETA (PICKUP-FLOOR/SLA-ANCHOR/COH-CLAMP/SEM-ETA/COH-R-DECLARED) ŚWIADOMIE POMINIĘTE — kalibracja ETA dojrzewa w cieniu (~10.07 może przedefiniować obietnicę), asercja czasu = strzał do ruchomego celu. Raport: `eod_drafts/2026-07-08/S_INWARIANTY_raport.md`.

> **Aktualizacja 2026-07-20 (OD-A1b, SOURCE-ONLY):** semantyka `_HARD` została domknięta jako **DROP-FERAL-CLAIM**, bez produkcyjnego `raise`: oba bliźniaki wołają wspólny `claim_ledger.check_feral_claim` przed skutkami proposal/live, odrzucają tylko naruszający claim i kontynuują tick. Log-loud oraz osobny `g_claim_ledger_feral_drops` trafiają do jsonl+summary przy HARD=ON. `_HARD` pozostaje default OFF; w tym zadaniu nie wykonano flipa, merge, deployu ani restartu. Strażnik: 24 testy, w tym OFF bajt-parytet, ON≠OFF, kontynuacja obu bliźniaków, metryka i mutation-probe dropu.
> **Aktualizacja 2026-07-08 (Sprint C route-order — ZERO zmian silnika/flag/flags.json; tylko dokumentacja+dowody):** uzbrojono 2 pary slotów **INV-SRC-ROUTE-ORDER 🔴→🟢** (Kontrakt ①, l.23) i **INV-TWIN-ROUTE-ORDER 🔴→🟢** (Kontrakt ③) — golden-fixture equivalence 3-noga (silnik/konsola/apka) była ZBUDOWANA (Sprint 0/30, 01-07.07) a slot STALE-🔴 (klasa C15: status z dashboardu przeterminowany). DOMKNIĘCIE: (a) mapa 4 kopii — silnik(route_order kanon)+plan_recheck(deleguje)+apka(deleguje, oba flagi LIVE) zwinięte do kanonu, panel = jedyna kopia golden-pinowana; (b) KAŻDA z 3 nóg golden mutation-probed RED (dispatch/panel/apka) + `test_route_order_live_parity` żywy ON — dowód że strażnik ma zęby (nie tekstowy, C13); (c) 0-diff dowód delegacji panelu (25 korpus + 9 żywych, także kolejność wewnątrz stopu) = flip-card gotowy, deploy za ACK. Siatka bez daty zastępuje wygasający `ziomek_time_route_monitor` (10.07). Raport: `eod_drafts/2026-07-08/S_C_ROUTEORDER_raport.md`.
...
- 🔴 **INV-FEAS-R6-ONE-SOURCE (re-spec ODR-001 2026-07-12):** jeden domenowy interwał `physical possession → customer handoff`, limit 35 normalnie / 40 tylko Alarm / nigdy klasa kuriera / `>40` niedopuszczalne. `food_ready_age` jest osobnym wymiarem bez przypisanego tu progu. Producer eventów, feasibility, route/plan, selection, serializer i oracle muszą importować ten sam kontrakt; click/last-inside/arrival nie mogą być cichym fallbackiem physical eventu.
  - 🟡 **DIAL-FAMILY consistency 2026-07-07 = strażnik baseline, nie ODR-001:** `test_inv_r6_dial_family` pilnuje równości kilku starych diali 35 oraz odrębnego best-effort 40. Nadal ma wartość anty-driftową, lecz nie dowodzi possession/handoff, automatycznego Alarmu ani zakazu klasowego wyjątku. Po implementacji ODR-001 wymaga re-spec/mutation probe dla obu trybów i eventów.
...
- 🟢 **INV-FEAS-NO-DOUBLE-BOOK** *(slot uzbrojony 2026-07-08; semantyka HARD domknięta 2026-07-20 jako DROP-FERAL-CLAIM; okno między tickami rozszerzone 2026-07-28)*: kurier nie może dostać 2. SPRZECZNEJ propozycji ani w jednym ticku, ani między tickami w oknie życia claimu. W jednym przebiegu de-konflikcji KOLEJNE claimy TEGO SAMEGO kuriera widzą worek rosnący DOKŁADNIE o +1 (poprzedni claim doklejony przez `claim_ledger.tentative_assign`); regres (flota niemutowana / pile-on) → worek nierosnący → naruszenie „stale". Między tickami, za `ENABLE_PROPOSAL_CLAIM_PERSISTENCE`, każdy aktywny wpis `pending_proposals` (`planned`, bez `courier_id`, `sent_at` młodszy niż `PROPOSAL_CLAIM_TTL_SEC`) musi wystąpić dokładnie raz w worku wskazanego kuriera z `commitment_level=proposed`; brak/duplikat/zły poziom albo błąd checkera jest naruszeniem `verify_proposal_claim_window`. Legalność kolejnego bundlingu nadal rozstrzyga kanoniczne feasibility, a gdy same miękkie claimy wyzerują pulę, obowiązuje jawny fallback bez claimów z `proposal_claims_relaxed=true` (always-propose pozostaje nadrzędne). Weryfikator `claim_ledger.verify_no_stale_claim` (leaf, pure), obserwator `check_sweep_trace` i wspólna bramka `check_feral_claim` są wpięte w OBA bliźniaki RAZEM: `pending_global_resweep.global_allocate` + `shadow_dispatcher._tick` (za `ENABLE_ENGINE_CLAIM_LEDGER`). Przy `_CHECK=ON,_HARD=ON` feralny claim dostaje werdykt `DROP_FERAL_CLAIM` przed skutkami proposal/live; pozostałe claimy oraz tick biegną dalej. Produkcyjnej ścieżki `raise` nie ma. Metryki `g_claim_ledger_breaches` i `g_claim_ledger_feral_drops` → jsonl+summary. `_CHECK=true` w flags.json od 08.07 14:14 (ACK Adriana); `_HARD` pozostaje default OFF i wymaga osobnego flipa ownera. Strażnik = `test_claim_ledger_no_double_book_inv` + `test_proposal_claim_persistence_etap1` (oracle między tickami, TTL, dedup, soft-relax i mutation probe). Zero-FP DOWÓD offline: 5000 losowych sweepów (3092 z legalnym bundlingiem, 22702 claimów) = 0 fałszywek; mutacja wykryta w 3630. Dowód live przed wyborem semantyki: 0 breach / 4231 wpisów / 245.9 h (`eod_drafts/2026-07-10/CLAIM_LEDGER_HARD_GATE_CARD.md`, regeneracja 18.07).

## Kontrakt ③ — PARYTET BLIŹNIAKÓW (z konstrukcji)
- 🟢 **INV-TWIN-ROUTE-ORDER** *(slot uzbrojony 2026-07-08, Sprint C)*: parytet bliźniaków silnik↔konsola↔apka = golden-fixture equivalence na WSPÓLNYM korpusie `tests/golden/route_order_corpus.json` (25 case'ów, ≥17 obowiązkowych klas czasówka/paczka/carried/trust-canon + żywe worki). 3 nogi (silnik/konsola/apka) KAŻDA mutation-probed RED 08.07 — patrz INV-SRC-ROUTE-ORDER. L6.A golden = fundament; siatka bez daty wygaśnięcia zastępuje `ziomek_time_route_monitor`.
...
- 🟡 **INV-TWIN-SLA-ANCHOR (IMPLEMENTED baseline, semantycznie superseded przez ODR-001):** wspólny `sla_anchor.py` pilnuje parytetu trzech bliźniaków starego anchora. To dowód spójności implementacji, nie poprawności product eventu; docelowy R6 anchor wymaga possession→handoff, a `food_ready_age` osobnego kontraktu.
...
**Wniosek:** ⚠️VOID = **0** (Sprint 1 Z1+Z2, 05.07: serializer + CONFTEST-STRIP + carried_first_guard + global_allocate — dowody `eod_drafts/2026-07-05/A1_{SERIALIZER_reoracle,INVARIANTS_devoid}_dowod.md`); **bramka `PENDING_RESWEEP_LIVE` ma podstawę pomiarową** (geometria w certyfikatorze + gate wpięty i testowany). Dług egzekwowania dalej w slotach ①②③ (5 z 16 — Sprint C 08.07 domknął route-order: INV-SRC-ROUTE-ORDER + INV-TWIN-ROUTE-ORDER 🟢, siatka golden bez daty zastąpiła monitor 10.07). Klasa DANE/STAN gęsto obstawiona.
