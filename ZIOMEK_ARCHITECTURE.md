...
- **Stan zleceń/planów:** `dispatch_state/orders_state.json`, `courier_plans.json`; domenowi ownerzy `state_machine`/`plan_manager`, a cała mechanika plikowa należy do `state_persistence`. `orders_state` ma osobną flagę default OFF: OFF deleguje dokładny legacy read/write i przepuszcza non-missing I/O bez alertu, ON wybiera walidowany fail-closed predecessor. Guard planu ON rozróżnia złą treść (recovery) od nieczytelności I/O (wyjątek, zero zapisu), przechodzi z SH na EX, fsyncuje cały syntetyczny zakres w JSON-safe version-HWM, następnie recovered main, i dopiero wtedy wydaje token CAS. Re-ON uzgadnia zdrowy main zapisany w okresie OFF z HWM pod EX; OFF nadal ignoruje `.prev`/HWM.
