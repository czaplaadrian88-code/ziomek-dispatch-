...
- **Stan zleceń/planów:** `dispatch_state/orders_state.json`, `courier_plans.json`; domenowi ownerzy `state_machine`/`plan_manager`, a cała mechanika plikowa należy do `state_persistence`. `orders_state` ma osobną flagę default OFF: OFF deleguje dokładny legacy read/write, ON walidowany fail-closed predecessor. Guard planu ON przechodzi z SH na EX, fsyncuje cały syntetyczny zakres w JSON-safe version-HWM, następnie recovered main, i dopiero wtedy wydaje token CAS; OFF ignoruje `.prev`/HWM także po rollbacku.
