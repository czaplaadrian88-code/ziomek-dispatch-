...
|---|---|---|---|
| **kolejność trasy (route-order)** | **4 kopie / 3 repa** *(korekta 06.07: 5. `courier_api_panelsync` USUNIĘTA — brak na HEAD)*: `plan_recheck._apply_canon_order_invariants` (silnik) ↔ `fleet_state._build_route` (konsola, NIE importuje) ↔ `route_podjazdy.order_podjazdy` (apka, deklaruje „jedyne źródło") ↔ Kotlin apka. Żywy rozjazd USTAŁ (monitor: 0 mismatch/d od 01.07, flagi trust-canon ON) — dług = parytet z FLAG, nie z konstrukcji | 1 moduł + golden-test parytetu | ⏰ **monitor wygasa 2026-07-10** → PoC route-order |
