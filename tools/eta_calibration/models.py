...
class EmpiricalQuantileModel:
    ...
    ODBIÓR:  target=poślizg; kontekst = (slot, load_bucket).
    DOSTAWA: target=czas dostawy; kontekst = (slot, load_bucket, dist_bucket).
    ...
    @staticmethod
    def _load_bucket(load):
        load = load or 1
        return min(4, int(load))
    ...
    def _ctx(self, r) -> tuple:
        lb = self._load_bucket(r.get("load"))
        if self.leg == PICKUP:
            return (r.get("slot"), lb)
        return (r.get("slot"), lb, self._dist_bucket(r.get("osrm_deliv_km")))
    ...
    def predict_quantiles(self, r) -> Dict[float, float]:
        ctx = self._ctx(r)
        base = self.ctx_q.get(ctx) or self.ctx_q.get((ctx[0], ctx[1])) or self.global_q
...
class LGBMQuantileModel:
    ...
    NUM_PICKUP = ["load", "hour", "wd", "otype", "hist_slip"]
    CAT_DELIV = ["cid_i", "rest_i"]
    NUM_DELIV = ["osrm_km", "osrm_ff", "load", "hour", "wd", "prep", "hist_pace"]
    ...
    def _row(self, r) -> Optional[list]:
        ...
        if self.leg == PICKUP:
            ...
            return [cid_i, rest_i, r.get("load") or 1, r.get("hour") or 0,
                    r.get("weekday") or 0, r.get("was_czasowka") or 0,
        ...
        return [cid_i, rest_i, r.get("osrm_deliv_km") or 0.0, r.get("osrm_deliv_ff_min") or 0.0,
                r.get("load") or 1, r.get("hour") or 0, r.get("weekday") or 0,
                r.get("prep_var_med") if r.get("prep_var_med") is not None else 10.0,
    ...
    def fit(self, train_rows: List[dict], chist: Dict[str, dict]) -> "LGBMQuantileModel":
        ...
        if self.leg == PICKUP:
            self.feat_names = ["cid_i", "rest_i", "load", "hour", "wd", "otype", "hist_slip"]
            self.cat_idx = [0, 1]
        else:
            self.feat_names = ["cid_i", "rest_i", "osrm_km", "osrm_ff", "load", "hour", "wd", "prep", "hist_pace"]
            self.cat_idx = [0, 1]
        X, y = [], []
        for r in train_rows:
            t = self._target(r)
...
def _rest_key(r) -> Optional[str]:
    # restauracja jako klucz kategorii — z coords (bez tekstu adresu w store); fallback prep bucket
