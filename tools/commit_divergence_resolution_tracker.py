"""Commit divergence resolution tracker (2026-05-27).

Koreluje:
- decyzje Ziomka `verdict=KOORD` z reason `commit_divergence_gate`
  — bramka uznała że plan ETA odbiega >threshold min od commit
  (czas_kuriera_warsaw), wycofała propozycję, NIE dotknęła panelu
- faktyczne `COURIER_ASSIGNED` events z audit_log po decyzji
  — Adrian/Bartek ręcznie przypisali w panelu

Output: `logs/commit_divergence_resolutions.jsonl` — per order:
  {ziomek_proposed: {cid, name, score, max_divergence_min, worst_oid,
                     threshold_min, czasowka_horizon_min},
   human_resolved: {cid, ts, source, delta_min},
   override: bool (ziomek_proposed_cid != human_resolved_cid),
   ziomek_ts, oid}

Cel analityczny: kalibracja progu (`COMMIT_DIVERGENCE_VERDICT_KOORD_MIN_MIN`,
default 10.0). Po 7-14d shadow data:
- < 5/dzień głównie 15+ min: próg OK, zostaw 10.0
- 5-20/dzień głównie 10-15 min, ratio override < 50%: za niski (false-positives
  na czasówkach z 60-min horyzontem) → bump do 15.0
- > 20/dzień głównie czasówki z dużym horyzontem: rozważ excluding czasówek

Idempotent: dedup po (oid, human_resolved_ts). Re-runs skipują zalogowane.

Usage:
  python3 -m dispatch_v2.tools.commit_divergence_resolution_tracker [--hours 24] [--dry-run]

Cwd: /root/.openclaw/workspace/scripts
Venv: /root/.openclaw/venvs/dispatch/bin/python

Cross-ref: tools/bug_e_resolution_tracker.py (mirror pattern),
tech_debt_backlog.md #BUG-C-TRACKER, Lekcja #149.
"""
import argparse
import json
import logging
...
RESOLUTION_LOG = Path("/root/.openclaw/workspace/scripts/logs/commit_divergence_resolutions.jsonl")

# Reasony Ziomka które = "bramka commit-vs-plan divergence wycofuje propozycję".
COMMIT_DIVERGENCE_REASON_PREFIXES = (
    "commit_divergence_gate",
)
...
_log = logging.getLogger("commit_divergence_resolution_tracker")
...
def is_commit_divergence_koord(rec: dict) -> bool:
    """Czy rec to verdict=KOORD z reason w commit_divergence_gate family."""
    if rec.get("verdict") != "KOORD":
        return False
    reason = rec.get("reason") or ""
    return any(reason.startswith(p) for p in COMMIT_DIVERGENCE_REASON_PREFIXES)


def extract_ziomek_proposed(rec: dict) -> dict:
    """Wyciąg z rec[best] + rec[commit_divergence_redirect] tego co Ziomek BY zaproponował."""
    best = rec.get("best") or {}
    redirect = rec.get("commit_divergence_redirect") or {}
    return {
        "courier_id": best.get("courier_id"),
        "name": best.get("name"),
        "score": best.get("score"),
        # Divergence-specific (z redirect payload)
        "max_divergence_min": redirect.get("max_divergence_min"),
        "worst_oid": redirect.get("worst_oid"),
        "threshold_min": redirect.get("threshold_min"),
        # Context heuristics dla calibration (czy czasówka z dużym horyzontem?)
        "is_czasowka": _is_czasowka(rec),
        "czasowka_horizon_min": _czasowka_horizon_min(rec),
    }


def _is_czasowka(rec: dict) -> bool:
    """Czy order_event w rec to czasówka (czas_odbioru >= 60 min)."""
    # decision_meta / order_event poles vary — best-effort
    auto_ctx = rec.get("auto_route_context") or {}
    if auto_ctx.get("auto_route_czasowka"):
        return True
    # Fallback: brak signalu = unknown
    return False


def _czasowka_horizon_min(rec: dict) -> Optional[float]:
    """Best-effort wyciąg czas_odbioru (prep) z rec — None gdy nie widać.

    Schema shadow_decisions może to mieć w różnych miejscach (best.metrics,
    order_event, etc.). Tracker zwraca pierwsze znalezione lub None.
    """
    best = rec.get("best") or {}
    # Niektóre wersje zapisują direct
    horizon = best.get("czasowka_horizon_min")
...
def collect_ziomek_koord_records(hours_back: int) -> Dict[str, dict]:
    ...
    for rec in iter_jsonl_records(str(SHADOW_LOG), cutoff_dt=cutoff):
        ...
        if ts is None or ts < cutoff:
            continue
        n_total += 1
        if not is_commit_divergence_koord(rec):
            continue
        n_match += 1
        ...
        if prev is None:
            ...
            if prev_ts is None or ts > prev_ts:
                by_oid[oid] = rec
    _log.info(
        f"scan shadow log: {n_total} records last {hours_back}h, "
        f"{n_match} commit_divergence_gate KOORD, {len(by_oid)} unique oids"
    )
    return by_oid
...
def main() -> int:
    ...
    if not ziomek_koords:
        _log.info("no commit_divergence_gate KOORD records — exit clean")
        return 0
