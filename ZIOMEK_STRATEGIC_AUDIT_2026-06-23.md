...
2. **TWARDE BRAMKI** (zmieniają FEASIBILITY = realny lewar, wyższe ryzyko):
   ...
   - **🟢 WYNIK 23.06 (`tools/r6_overpessimism_test.py`) — R6=35 ZA OSTRY (pierwszy GREEN lewar kampanii):** kalibracja `r6_max` × realny breach jest PŁASKA do 45: [30,35)→11,0% · **[35,40)→11,4%** (≈ baseline 11%, ≈ feasible) · [40,45)→13,3% · breach skacze dopiero **[45+]→19,1%**. Bramka = **85% fałszywych alarmów** przy progu 35 (815 false / 147 trafnych). Relaks R6→40 uratowałby **29% best-effortów** (61/209) — dając feasible kuriera zamiast wymuszonego złego picku — admittując kandydatów o realnym breach 11,4%. **REKOMENDACJA: ZMIĘKCZYĆ R6 — hard-reject dopiero ~42-45 (realna strefa zagrożenia), 35-42 jako feasible-z-karą-soft** (rozszerzyć istniejącą strefę soft 30-35). ⚠ caveaty: `r6_max`=bag-max proxy (nie per-order); R6 to BUSINESS rule R-35MIN-MAX → **decyzja Adriana czy commit 40 min OK**; obserwacyjne → wymaga **shadow-A/B w feasibility_v2 + monitor realnego breach** przed flipem.
   **PREDYKCJA NIEREDUKOWALNA przy decyzji (`tools/prediction_reducibility_test.py`, insight Adriana „urealnić czasy zamiast zmiękczać R6"):** delivery-pred median err **−1,7 min** (już dokładna/konserwatywna, NIE zaniża delivery-legu); tylko **3/39 restauracji** systematycznie niedoszacowane (2 = paczki=artefakt, 1 food low-breach); zaskoczenia naprawialne per-restaurant priorem = **3,1%**, reszta **96,9% = LOSOWY prep-slip** (nie per-restauracja). → Insight Adriana JEST słuszny w zasadzie (dokładna predykcja → R6=35 zostaje, ogon maleje), ALE „dokładniejsze czasy” to problem **DANYCH** (real-time „gotowe” z restauracji), NIE modelu — przy decyzji ta informacja NIE ISTNIEJE (96,9% slipu nieprzewidywalne). **Wniosek: real-time-ready = root fix (§18.3 #3, teraz potrójnie zwalidowany); zmiękczenie R6 = jedyny DECISION-TIME workaround; per-restaurant prior = bez wartości (3%, głównie paczki).**
...
1. **🎯 ANALIZA OGONA PORAŻEK — rekomendacja #1.** Mediana nasycona/dobra → cała wartość w ~11% ZŁYCH dostaw (realny R6 breach). Zbadać co je ODRÓŻNIA: saturacja (pool=0)? konkretne restauracje (wolny prep)? strefy/dystans? godziny? czasówki? **Wzorzec adresowalny** (restauracja X zawsze breach / strefa Y) → handle specjalnie. **Czysta saturacja** → capacity (nie-algorytm). Mówi GDZIE realnie działać — zamiast stroić nasyconą medianę.
   **WYNIK 23.06 (`tools/failure_tail_analysis.py`, czerwiec, 515 porażek/4645 = 11,1%):** (a) **71,5% = ZASKOCZENIA** (Ziomek przewidział on-time, real +31 min mediana — luka ETA/prep), 28,5% = przewidziane (capacity). (b) **~44% porażek = saturacja** (pool feasible ≤1, brak wyboru = capacity, nie-algorytm). (c) **Koncentracja na WOLNYCH restauracjach** (rate vs baseline 11%): Eat Point 27%, Chinatown 17%, Pizza Dealer 16%, Mama Thai 14%, Chicago Pizza 13%; top10 = 49% porażek. (d) **R6-bramka OVER-PESYMISTYCZNA** — 815 „przewidzianych breach" dostarczyło na czas (vs 147 trafnych → ~85% fałszywych alarmów). (e) bundle/bag NIE są driverem (solo 12,5% ≈ bundle 10,5%). ⚠ paczki (Dr Tusz 49%, Nadajesz.pl 36%) = ARTEFAKT (brak deadline'u termicznego) — wykluczyć z analizy food. **→ ADRESOWALNY LEWAR: per-restaurant prep/committed-time dla ~5 wolnych restauracji** (niuansuje prep_bias R²≈0 NO-GO: ŚREDNIA = szum, ale OGON koncentruje się na konkretnych restauracjach → interwencja celowana w te restauracje, nie globalna korekta). Reszta ogona = saturacja (capacity).
   **SHADOW-SYGNAŁY vs ogon (`tools/shadow_signals_vs_tail.py`, 23.06): ŻADEN liczony sygnał NIE łapie ogona.** ETA R3 (korekta LightGBM) recall **4%** na porażkach (GORSZY niż baza 38% — R3 ściąga predykcje ku medianie, anty-pomocny dla ogona). Redirecty/anomalie (pickup_extension 23 vs 27%, prep_bias≥12 47 vs 45%, ALERT 55 vs 52%) NIE dyskryminują. Jedyny dyskryminujący = **best_effort/pool=0** (21,5% vs 10,5% = 2× = marker SATURACJI/capacity, NIE lewar — gdy pool=0 nie ma lepszej opcji). **Wniosek: ZASKOCZENIA są NIEPRZEWIDYWALNE przy decyzji** (prep-slip dzieje się PO decyzji, nieforecastowalny z cech) → jedyny realny fix = **real-time „gotowe" z restauracji** (§18.3 #3, teraz MOCNO zwalidowany jako fix na 71,5% zaskoczeń). Per-restaurant = słaba base-rate (Chinatown 17% vs 11%), nie izolowalna per-order.
