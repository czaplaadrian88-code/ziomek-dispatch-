...
DIAGNOSTIC_MARKER = "A3_BEST_OF_WORST"
...
NON_EMPTY_FLEET_WITHOUT_OBSERVABLE_CID = (
    "non_empty_fleet_without_observable_cid"
)
INTERNAL_ERROR = "a3_internal_error"
SELECT_ATTACHMENT_ERROR = "a3_select_attachment_error"
_FAILURE_CODES = frozenset({
    NON_EMPTY_FLEET_WITHOUT_OBSERVABLE_CID,
    INTERNAL_ERROR,
    SELECT_ATTACHMENT_ERROR,
})


class _AlwaysProposeInvariant(ValueError):
    """Wewnętrzny błąd kontraktu zamieniany na jawny wynik diagnostyczny."""
...
def _public_candidate(candidate: Any) -> dict:
    ...
    return {
        ...
        "a3_solo_score": (
            _score(candidate) if pickup_distance is not None else None
        ),
        "has_plan": getattr(candidate, "plan", None) is not None,
    }


def _physical_fleet_count(value: Any) -> int:
    try:
...
def build_failure_diagnostic(*, fleet_count: Any, code: str) -> dict:
    """Minimalny deterministyczny fallback bez exception text i bez PII."""
    safe_code = (
        code
    ...
    result = {
        ...
        # Po wyjątku nie wolno udawać, że znamy liczności częściowej ewaluacji.
        "observed_candidate_count": None,
        "hard_safe_count": None,
        "soft_evidence_complete": False,
        "proposal_output_type": proposal_output.COORDINATOR_ESCALATION,
        "recommend_only": True,
        "candidate": None,
        "diagnostic": {"status": "FALLBACK", "code": safe_code},
    }
    if safe_code == NON_EMPTY_FLEET_WITHOUT_OBSERVABLE_CID:
        result["invariant_violation"] = safe_code
    else:
        result["diagnostic_error"] = safe_code
    return result


def _build_best_of_worst(
    candidates: Iterable[Any], *, physical_fleet_count: int
) -> dict:
    """Część polityki, której każdy wyjątek przechwytuje publiczna granica."""
    unique = _deduplicate(candidates)
    if physical_fleet_count > 0 and not unique:
        raise _AlwaysProposeInvariant(
            NON_EMPTY_FLEET_WITHOUT_OBSERVABLE_CID
        )
...
def build_best_of_worst(
    candidates: Iterable[Any],
    *,
    fleet_count: int,
    on_error: Callable[[str, str], None] | None = None,
) -> dict:
    """Zwróć stabilną i totalną podpowiedź bez zmiany decyzji wykonawczej.

    HARD-safe kandydat zawsze wygrywa z HARD-NO. Jeśli polityka lub jej dane
    zawiodą, wynik pozostaje jawną ``COORDINATOR_ESCALATION`` z bezpiecznym
    kodem diagnostycznym; żaden wyjątek instrumentu nie wychodzi do selection.
    Callback logujący jest best-effort i również nie może złamać totalności.
    """
    physical_fleet_count = 0
    try:
        physical_fleet_count = _physical_fleet_count(fleet_count)
        return _build_best_of_worst(
            candidates,
            physical_fleet_count=physical_fleet_count,
        )
    except Exception as exc:
        code = (
            exc.code
            if isinstance(exc, _AlwaysProposeInvariant)
            else INTERNAL_ERROR
        )
        if on_error is not None:
            try:
                on_error(code, type(exc).__name__)
            except Exception:
                pass
