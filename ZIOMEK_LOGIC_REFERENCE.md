...
- `ENABLE_CZASOWKA_CK_MANUAL_EDIT_PASSTHROUGH` — **LIVE (flip ON 20.07 ~14:45, incydent pilota 489052: owner ręcznie poprawił odbiór 12:01→11:41 w gastro, guard pasywny tłumił zmianę, kurier odebrał FAKTYCZNIE 11:40)**. Mechanizm: potwierdzona ręczna korekta CK czasówki (krawędź markera gastro `zmiana_czasu_odbioru` False→True + `pickup_at` niezmieniony w tym samym odczycie + status panelu zgodny z klasą stanu) jest tłumaczona na kanoniczny `PICKUP_TIME_UPDATED` (`state_machine.build_czasowka_manual_ck_pickup_event`) zamiast `CK_PASSIVE_SUPPRESSED`. Śmieciowy re-stamp gastro przy zmianie statusu dalej blokowany W OBU trybach (None/legacy marker = fail-closed). Bliźniaki: state_machine + panel_watcher (fetch markera z tego samego response) + dispatch_pipeline pre_proposal_recheck (event `PICKUP_TIME_UPDATED_PRE_RECHECK_CK_MANUAL`). Testy `test_czasowka_pickup_authority.py`. OFF = bajt-parytet sprzed zmiany. Rollback hot = klucz false. Autor: Codex/Sol, apply+flip CTO za standing ACK ownera (commit `188db47`).
