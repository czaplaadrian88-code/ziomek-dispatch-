...
| `ENABLE_FULL_CHOICE_SET_LOG` | `core.selection` → `PipelineResult.full_pool_candidates` → `shadow_dispatcher._serialize_result` | Choice-set (21.07): ON dopisuje do decyzji `full_pool_compact` dla całej puli sprzed top-16; kandydat ma wyłącznie `{cid, score, feasibility_verdict, pos_source, km_to_pickup, r6_bag_size}`. Wspólny projection zasila LOCATION A alternatives i LOCATION B inline-best; OFF = brak nowego klucza, default OFF, rollback hot |
