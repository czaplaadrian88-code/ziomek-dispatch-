...
> | `ENABLE_CZASOWKA_RUTCOM_FORWARD_AUTHORITY` | — | **KANDYDAT v20/OFF** (02.08, incydent 491578; owner ACK na ON po wdrożeniu) | czasówka: bezpieczny ruch Rutcom CK do przodu przy aktywnym `planned`/`assigned` i `status_id=2` staje się kanonicznym `PICKUP_TIME_UPDATED`; wspólny próg 3 min, queue-bound receipt v5, immutable claim/successor/replay, courier/assignment oraz pickup+CK revision CAS, sprzężone pola i exact attestation. V20 zapisuje initial tuple jako hash-bound durable intent razem z `NEW_ORDER` shellem; czysty resolver bez runtime flag dokańcza dokładnie tę transakcję także po ON→OFF, a state atomowo materializuje pickup+CK+HH:MM+provenance i konsumuje receipt. Restart odzyskuje intent przed świeżym panelem. Complete-contract wymaga braku pending intentu, a forward fence blokuje każdy pending writer czasu i każdy czasowy `NEW_ORDER`; wyjątek obejmuje tylko fully-bound explicit elastic, code revert nadal failuje closed. Review v19 wykryły cztery luki jako 4/4 RED; v20 ma sześć mutation kills, 7/7 exact restore, 592/592 focused i pełną regresję `6617P/0F/74S/8X/149W` w 594,91 s. Dwa final-byte `CLEAN` pozostają wymagane; preflight 16:31Z miał dwie aktywne legacy czasówki, więc flip pozostaje HOLD do ich terminalizacji i ponownego quiesced statusu |
