...
> | `ENABLE_EXPLICIT_UNKNOWN_POSITION_MODEL` | ⚪ | **KANDYDAT/OFF** (22.07) | następca `ENABLE_NO_GPS_NEUTRAL_SCORE_DIST`: jawny model UNKNOWN Białystok v1 (6.5 km SOFT, 15 min SOFT, 22 min HARD) współdzielony przez scoring, feasibility i symulator trasy. Konflikt ze starą neutralizacją fail-closed. Default OFF; flip wymaga osobnego ACK |
> | `ENABLE_EXPLICIT_UNKNOWN_POSITION_SHADOW` | ⚪ | **SHADOW/OFF** (22.07) | osobna bramka kosztownego dual-eval legacy/explicit oraz drugiego rzeczywistego selektora; zapisuje porównanie obu polityk, ale sama nie zmienia zwycięzcy. Twin `ENABLE_EXPLICIT_UNKNOWN_POSITION_MODEL`; default OFF, rollback OFF = zero dual-eval/payloadu |
...
> | `ENABLE_GPS_MERGE_LOCK` | — | **flags.json=ON** (snapshot 02.08; skuteczny rollout weryfikuje workstream A-1) | default OFF; wspólny writer `gps_pwa_store` serializuje courier-api, dispatch-gps i GC dedykowanym lockfile oraz robi merge-by-timestamp, kończąc cross-process lost-update. OFF zachowuje bajtowy legacy zapis. Rollback: OFF i kontrolowany restart obu writerów za ACK; ON wymaga obu stron na tym samym kodzie, kill-testu i 2 dni obserwacji `gps_pwa_cid_vanished_total==0` |
> | `ENABLE_NOTIFY_CHANNEL_SPLIT` | — | **flags.json=ON** (snapshot 02.08; skuteczny proces potwierdza owning rollout) | techniczne alerty kieruje do kanału NAPRAW, biznesowe/critical zawsze zostawia na głównej grupie; nieznany input i błąd pobocznego transportu są fail-open na główny kanał. OFF/OFF/OFF ma exact legacy schema/call graph. Rollback hot: flaga OFF; sekretów kanału nie dokumentujemy |
...
> | `ENABLE_CZASOWKA_RUTCOM_FORWARD_AUTHORITY` | — | **KANDYDAT v22/OFF** (02.08, incydent 491578; owner ACK na ON po wdrożeniu) | czasówka: bezpieczny ruch Rutcom CK do przodu przy aktywnym `planned`/`assigned` i `status_id=2` staje się kanonicznym `PICKUP_TIME_UPDATED`; wspólny próg 3 min, queue-bound receipt v5, immutable claim/successor/replay, courier/assignment oraz pickup+CK revision CAS, sprzężone pola i exact attestation. Durable initial intent jest bound do applied outboxa `NEW_ORDER`; recovery działa przed każdym writerem lifecycle. V22 ma jeden post-event projector dla state/preflight oraz jeden request-scoped snapshot polityki sprzed HTTP, przekazywany aż do durable apply: OFF→ON nie nadaje authority, ON→OFF domyka rozpoczęty event. Review v21 dały 4/4 RED; v22 ma 4/4 PASS, dwa dodatkowe hot-flip oracles, pięć mutation kills, 226/226 focused, 456/456 broad i pełną regresję `6713P/0F/74S/8X/153W` w 637,32 s. Lifecycle repo/live 557/557, hygiene 271/271. Live preflight ma queue/outbox/incomplete state = 0; jedyny HOLD to aktywne dwa writery, więc po dwóch final-byte `CLEAN` wymagane jest kontrolowane quiesce i ponowny zielony `forward-status --quiesced` przed deployem/flipem |
...
- `ENABLE_PROPOSAL_CLAIM_PERSISTENCE` — ETAP 1 proposal-claims (2026-07-28, konsensus Sol+Opus+Fable, gate `engine.proposal-claim-persistence`): trwały claim WISZĄCYCH propozycji MIĘDZY tickami. Na starcie `_tick` aktywne wpisy z istniejącego `pending_proposals.json` (status `planned` bez `courier_id`, wiek `sent_at` < `PROPOSAL_CLAIM_TTL_SEC`, default 240 s) doklejane do snapshotu floty przez `claim_ledger.tentative_assign(..., commitment_level="proposed")` — kurier z wiszącą propozycją przestaje wyglądać na pustego (root cause kolizji „3 odbiory z różnych miejsc o tej samej porze", 53% ruchu w pile-on). Semantyka MIĘKKA: gdy same claimy wyzerują pulę feasible → powtórna ocena bez claimów (`proposal_claims_relaxed=true`), PROPOSE nigdy nie degraduje do KOORD (always-propose nadrzędne). Telemetria: `proposal_claims_count` (jawnie serializowane, lekcja #80), `bag_size_before` = NADAL realny worek. Strażnik: `verify_proposal_claim_window` (INV-FEAS-NO-DOUBLE-BOOK rozszerzony poza tick; błąd checkera = naruszenie). Zero nowego writera — źródłem jest pending_proposals (prekursor kontraktu `proposal_assignment` Etapu 2). Default OFF = bajt-parytet; hot-reload; rollback = false bez restartu.
- `PROPOSAL_CLAIM_TTL_SEC` — TTL trwałego claimu propozycji (default 240 s ≈ p90 reakcji koordynatora 4,27 min); numeric override w flags.json (hot).
...
- `ENABLE_PLAN_CORRUPT_RAISE` — **A-2 iter4 03.08, KANDYDAT SOURCE-ONLY, default OFF, bez flipa/deployu/restartu.** ON: `plan_manager` czyta `courier_plans.json` przez jednego ownera `state_persistence`; `.prev` jest walidowanym poprzednikiem, którego brak/uszkodzony main nie nadpisuje. Recovery jest dozwolone wyłącznie po dowodzie braku albo złej treści maina (`JSONDecodeError`/encoding/root-shape), nigdy po wyczerpanym `OSError`: błąd I/O wychodzi do callera, a zdrowy nieodczytany main pozostaje nietknięty. Reader wymagający recovery albo uzgodnienia wersji zwalnia `LOCK_SH`, bierze `LOCK_EX` i czyta ponownie. Recovery pod EX fsyncuje cały deterministyczny zakres tokenów `>HWM`, utrwala recovered main i dopiero potem zwraca widok. Re-ON po legalnym oknie OFF uzgadnia brakujący/stary poprawny HWM do maksimum zdrowego maina pod EX, bez zmiany maina/`.prev`; malformed HWM nadal fail-closed. Epoka mieści się w IEEE-754 safe integer. `world_record` przy ON snapshotuje main+HWM spójnie; replay materializuje scratchową parę przez tego samego ownera. OFF pozostaje pełnym legacy: nie czyta ani nie zapisuje `.prev`/HWM, strict missing zwraca `{}`, root-array ma dokładny historyczny `ValueError`, a wersja to `current+1`. Rollback i ponowny ON są bezstratne przy pozostawionym albo usuniętym HWM. Flip/blind należą do CTO/ownera.
- `ENABLE_ORDERS_STATE_PERSISTENCE_V2` — **A-2 iter4, default OFF, osobna bramka dla `orders_state`.** Jeden chokepoint `state_machine._write_state` obejmuje wszystkie RMW oraz bulk-prune. OFF deleguje do jednego ownera dokładny historyczny writer/read: format bajtów z `indent=2`, best-effort `.prev`, zapis mimo uszkodzonego maina/awarii backupu i dowolny JSON root; każdy `OSError` poza `FileNotFoundError` przechodzi bez konwersji i bez nowego alertu. ON deleguje hardened object-only read, retry I/O, `StateReadError`+alert oraz walidowany fail-closed predecessor/write. Pozostałe po ON sidecary są dla OFF inert; rollback przywraca byte-parity bez kasowania plików. Flip wymaga osobnego ACK ownera.
...
### Sprint CK-MANUAL-EDIT (2026-07-20) — ręczna korekta czasu czasówki przechodzi do silnika
- `ENABLE_CZASOWKA_CK_MANUAL_EDIT_PASSTHROUGH` — **LIVE (flip ON 20.07 ~14:45, incydent pilota 489052: owner ręcznie poprawił odbiór 12:01→11:41 w gastro, guard pasywny tłumił zmianę, kurier odebrał FAKTYCZNIE 11:40)**. Mechanizm: potwierdzona ręczna korekta CK czasówki (krawędź markera gastro `zmiana_czasu_odbioru` False→True + `pickup_at` niezmieniony w tym samym odczycie + status panelu zgodny z klasą stanu) jest tłumaczona na kanoniczny `PICKUP_TIME_UPDATED` zamiast `CK_PASSIVE_SUPPRESSED`. Od sprintu RUTCOM-COMMITTED-AUTHORITY kompatybilny symbol `state_machine.build_czasowka_manual_ck_pickup_event` wyłącznie deleguje do jednego ownera polityki w `committed_pickup_authority.py`; nie zawiera drugiej kopii reguł. Śmieciowy re-stamp gastro przy zmianie statusu dalej blokowany W OBU trybach (None/legacy marker = fail-closed). Bliźniaki: state_machine + panel_watcher (fetch markera z tego samego response) + dispatch_pipeline pre_proposal_recheck. Testy `test_czasowka_pickup_authority.py`. OFF = bajt-parytet sprzed zmiany. Rollback hot = klucz false. Autor: Codex/Sol, apply+flip CTO za standing ACK ownera (commit `188db47`).

### Sprint RUTCOM-COMMITTED-AUTHORITY (2026-08-01) — umówiony czas 19:21 nie może zostać w aplikacji jako 19:16
- `ENABLE_CZASOWKA_RUTCOM_FORWARD_AUTHORITY` — **KANDYDAT v20, default OFF; owner 01.08 wydał ACK na docelowe ON i wymagane kontrolowane restarty po wdrożeniu kodu**. Root cause incydentu 491578: Rutcom podał 19:21, lecz guard potraktował CK bez markera jak statusowy re-stamp i 52 razy stłumił 19:16→19:21; aplikacja poprawnie wyświetlała stary kanon. `committed_pickup_authority.py` jest jednym policy/CAS ownerem obu pól, `committed_pickup_apply.py` jednym transportem, a `PICKUP_TIME_UPDATED` jedynym writerem atomowego pickup+CK+HH:MM+provenance. Forward wymaga aktywnej czasówki, statusu Rutcom 2, ruchu ≥3 min do przodu i w przyszłość, flagi oraz passive guarda; proof wiąże klasę, courier/assignment, pickup+CK revisions i wszystkie sprzężone pola, a kolejka v5 zachowuje exact claim/successor/poison. V20 zapisuje pierwotny initial tuple przed sanityzacją jako hash-bound `committed_pickup.new_order_intent.v1` w tej samej transakcji co `NEW_ORDER` shell. Jeden czysty resolver bez runtime flag materializuje z receiptu najwyżej jeden `PICKUP_TIME_UPDATED`, także po ON→OFF; atomowy writer zapisuje pełny kontrakt i konsumuje intent. Restart tick odzyskuje receipt przed świeżym panelem, więc statusowy restamp 19:16 nie zastępuje umówionego 19:21. Complete-contract wymaga braku pending intentu, a forward preflight blokuje każdy pending writer czasu i każdy czasowy `NEW_ORDER`; wyjątek dotyczy wyłącznie w pełni związanego jawnego elastyka, code revert nadal failuje closed. Review v19 wykryły cztery luki przed live i wszystkie odtworzono RED. V20 ma sześć mutation kills, exact restore 7/7, focused 592/592; pełna regresja i dwa świeże `CLEAN` final-byte pozostają wymagane. `forward-status` 16:31Z miał pustą kolejkę i outbox, lecz dwie aktywne legacy czasówki z rozbieżnym pickup/CK, więc flip pozostaje HOLD do ich naturalnej terminalizacji, quiesce i ponownego zielonego preflightu. Rollout: zatrzymać panel-watcher+shadow, zielony preflight, backup/deploy, ustawić flagę ON przed startem nowych procesów. Rollback hot=`false`; code revert tylko przez narzędzie po pełnym fence/backup/outbox/state preflight. Pełny kontrakt: `docs/RUTCOM_COMMITTED_PICKUP_AUTHORITY.md`.

### Sprint CZASOWKA-RECLAIM-SHADOW (2026-07-21) — pomiar przekroczenia granicy czasówki
- `ENABLE_CZASOWKA_RECLAIM_SHADOW` — **default OFF, wyłącznie SHADOW**: durable downstream `PICKUP_TIME_UPDATED`, dopiero po zapisie nowego `pickup_at_warsaw`, zapisuje idempotentny rekord `czasowka_reclaim_shadow.jsonl` i licznik `would_reclaim` per `(oid, assignment_event_id)`. Trigger to dokładnie `old <= observed_at+CZASOWKA_PREP_MIN(60)` oraz `new >= observed_at+60+CZASOWKA_RECLAIM_HYSTERESIS_MIN(5)` oraz `new > old`; pełne guardy stanu i przyczynowości są logowane. OFF = zero I/O; nie zmienia state, planu ani gastro. `ENABLE_CZASOWKA_RECLAIM_LIVE` — **default OFF i nieuzbrojona**: istnieje konstruktor oraz uśpiony handler state eventu `ORDER_RECLAIMED_TO_CZASOWKA`, ale nie ma producenta/callera ani wymaganych writerów gastro/planu. Sam handler po jawnie autoryzowanym evencie potrafi zmienić `orders_state`, więc nie jest bezpiecznym ani kompletnym LIVE. Etap LIVE wymaga osobnej karty i ACK ownera. Rollback SHADOW = klucz false/brak klucza; obserwacja przed werdyktem ≥2 dni.
...
- **KOD GAŁĘZI USUNIĘTY 2026-07-20** (gated apply po zielonym oknie at#217; pre-ACK Adriana 18.07 „Dawaj usunięcie kodu gałęzi po oknie"): obie gałęzie w `feasibility_v2` wycięte (markery-komentarze zostawione w miejscu), flaga zdjęta z `ETAP4_DECISION_FLAGS`+const w common, klucz usunięty z flags.json, lifecycle registry → `dead`. Strażnik post-removal: `test_d3_gold_quantile_flip.py` (gold=std parytet klas + **flaga-widmo w flags.json = inert** — ochrona przed cichą reanimacją wyjątku). Rollback = `git revert` commitu cleanup.
...
- `ENABLE_BEST_EFFORT_ESC_TRUSTED_ETA` — **SHADOW/LOG-ONLY (owner 19.07, GO 21.07; default OFF):** OFF zachowuje bezwarunkowy `BEST_EFFORT_ESC_TIER2_MAX_FREE_MIN` (dziś 90) bajt-w-bajt; ON daje Tier 2 próg 90/configured tylko gdy kandydat ma znaną i jawnie niesyntetyczną pozycję, `eta_calib` delivery `n_pace≥30`, integralny aktualny champion oraz świeży (≤36 h) rolling OOT tej samej rodziny z MAE≤8 i n≥200; każdy brak/niespójność → 30. Pure sygnał i loader: `eta_trust.py`; `eta_trust_*` płynie per kandydat przez wspólny serializer A+B, a reader `tools/best_effort_escalation_report.py` obejmuje legacy90 oraz trusted90/30 (nie ukrywa fallbacków 30). Nie zmienia best/plan/score/feasibility/verdict; zależy od istniejącego `ENABLE_BEST_EFFORT_OBJM_SHADOW`. Rollback hot = false/brak klucza.
...
| `ENABLE_REASSIGN_OLD_PLAN_RELEASE` | panel_watcher:`_release_plan_on_reassign` (branch reassign + PANEL_PACKS FALLBACK) oraz `disappeared → ORDER_RETURNED_TO_*` | po przerzuceniu lub zwrocie znikniętego zlecenia zwalnia plan STAREGO kuriera: `remove_stops` (bump plan_version → apka starego od razu przestaje pokazywać zabrane/zwrócone zlecenie, zamiast czekać na fallback 180 s / 5-min plan_recheck) + recanon reszty worka (`reassign_out`/`return`); best-effort; default OFF, flip za ACK |
| `ENABLE_STATE_OUTBOX_SWEEPER` | panel_watcher:`_sweep_state_apply_outbox` (tick, przed fetchem) | E1 crash-consistency: co tick domyka wiszące wpisy `state_apply_outbox` (events.db) starsze niż `STATE_OUTBOX_SWEEPER_MIN_AGE_S` (30 s), limit 100/tick, metryka `state_outbox_sweeper_completed` w logu; OFF = zero dodatkowego I/O, zostaje tylko legacy drain po udanym fetchu; default OFF, flip za ACK |
| `ENABLE_A360_D1_OD07_FIREWALL_EXEMPT_TRUTH` | dispatch_pipeline (fail-closed odczyt, ~:4185) | rule_verdict.v3 pod OD-07: R6=possession→handoff, 35 norma/40 tylko-Alarm, brak eventów fizycznych → jawny UNBOUND/HOLD; observability-only (enforcement=NONE), OFF = v1 bajt-w-bajt; FLIPNIĘTA ON 21.07 za ACK ownera |
...
| `ENABLE_UWAGI_BRIDGE_NADAWCA` | external `drtusz_bridge.build_uwagi` patch → `uwagi_bridge_envelope` → `panel_watcher` → `inspect_bridge_nadawca` | P0 bridge-NADAWCA v2 (22.07): pickup tylko z terminalnej koperty HMAC-SHA256; materiał HMAC z konfigurowalnego pliku 0600 poza repo. Producent percent-escapuje `|`/`%`/CR/LF w każdym polu, konsument liczy surowe `NADAWCA:` (=1), weryfikuje podpis stałoczasowo, granicę `Odbiorca:`, jedną kotwicę pocztową i bbox geokodu. Flaga jest runtime- i registry-związana z `ENABLE_FIRMOWE_REJECT_ON_GEOCODE_FAIL=true`; brak materiału, v1, zły podpis, niejednoznaczność lub geocode/bbox fail → legacy/KOORD bez centrali. PII-free wynik per order trafia do rotowanego `uwagi_bridge_envelope.jsonl`. Default OFF; flip tylko za ACK po review i obserwacji >=2 dni. |
| `ENABLE_UWAGI_BRIDGE_NADAWCA_V1` | historyczny marker tekstowy | WYCOFANE/NO-GO: v1 nie dowodził pochodzenia i pozostaje odrzucany do legacy/KOORD. |
| `ENABLE_FULL_CHOICE_SET_LOG` | `core.selection` → `PipelineResult.full_pool_candidates` → `shadow_dispatcher._serialize_result` | Choice-set (21.07): ON dopisuje do decyzji `full_pool_compact` dla całej puli sprzed top-16; kandydat ma wyłącznie `{cid, score, feasibility_verdict, feasibility_reason, pos_source, km_to_pickup, r6_bag_size}`. `feasibility_reason` (7. pole, 04.08) = dosłowny powód z kandydata, czyli bramka, która ustaliła finalny werdykt (`check_feasibility_v2` wraca na PIERWSZEJ odrzucającej bramce; post-bramki `core.candidates` przepisują parę verdict+reason razem); `None` dla feasible (`MAYBE`), bo „nie wycięty" niesie już `feasibility_verdict`. Wspólny projection zasila LOCATION A alternatives i LOCATION B inline-best — obie czytają klucze jawnie, więc 7. pole ich nie zmienia (parytet bajtowy A/B); OFF = brak nowego klucza, default OFF, rollback hot |
| `ENABLE_LEARNING_LOG_DECISION_JOIN` | `panel_watcher` writerzy `PANEL_AGREE` + `PANEL_OVERRIDE` | Join-hardening (21.07): ON zapisuje `learning_log.lifecycle_event_id = decision_record.event_id` (ten sam klucz co `shadow_decisions.event_id`); późniejsze ID `COURIER_ASSIGNED` pozostaje jako `assignment_lifecycle_event_id` dla idempotencji outboxa. OFF zachowuje kontrakt E1; stare rekordy bez klucza nadal są czytelne; default OFF, rollback hot |
