...
> | `ENABLE_CZASOWKA_RUTCOM_FORWARD_AUTHORITY` | — | **KANDYDAT v18/OFF** (02.08, incydent 491578; owner ACK na ON po wdrożeniu) | czasówka: bezpieczny ruch Rutcom CK do przodu przy aktywnym `planned`/`assigned` i `status_id=2` staje się kanonicznym `PICKUP_TIME_UPDATED`; wspólny próg 3 min, queue-bound receipt v5 z osobnym czasem kliknięcia/wykonania i kompatybilnością v4, immutable claim/successor/replay, rzeczywista klasa, courier/assignment oraz pickup+CK revision CAS, old-CK i sprzężony order_type/prep/deadline/marker CAS+postcondition, exact outbox attestation; v18 ma jeden assignment CK resolver z durable snapshotem flag, jeden pickup null/value path z legalnym pierwszym `None` baseline, pełny cold-start `NEW_ORDER→COURIER_ASSIGNED`, rollback TTL od `eligible_at`, poison-preserving re-click oraz `forward-status` schema v3 blokujący całą klasę unfinished authority i niepełny kontrakt wg wspólnego classifiera, bez fallbacku runtime; ACK dopiero po exact terminalnym outbox state+downstream; post-pickup/stale generation/ABA/cofnięcie bez autorytetu pozostają zablokowane |
...
- `ENABLE_CZASOWKA_RUTCOM_FORWARD_AUTHORITY` — **KANDYDAT v18, default OFF; owner 01.08 wydał ACK na docelowe ON i wymagane kontrolowane restarty po wdrożeniu kodu**. Root cause incydentu 491578: Rutcom podał 19:21, lecz guard potraktował CK bez markera jak statusowy re-stamp i 52 razy stłumił 19:16→19:21; aplikacja poprawnie wyświetlała stary kanon. Naprawa u źródła: `committed_pickup_authority.py` jest jedynym policy/CAS ownerem obu pól; `committed_pickup_apply.py` kanonizuje legalny CK przed jednym outboxem; `PICKUP_TIME_UPDATED` jest jedynym writerem atomowego pickup+CK+HH:MM+provenance. Automatyczny forward wymaga aktywnej czasówki, Rutcom status 2, ruchu ≥3 min do przodu i w przyszłość, nowej flagi oraz passive guarda. Proof/event/postcondition wiążą rzeczywistą klasę, oba courier IDs, assignment, pickup revision, CK revision, stary pickup/CK oraz jedną kanoniczną mapę old/new `order_type`, prep, deadline i markera; przegrany CAS lub ABA jest `superseded`. Queue-bound receipt v5 zachowuje immutable click audit i zaczyna TTL successora dopiero przy promocji, a projekcja rollbacku zachowuje tę epokę przez `eligible_at`; v4 pozostaje czytelne. Re-click przed zapisem weryfikuje exact claimed head i successor, więc nie nadpisuje poison evidence. Assignment CK ma jeden resolver i receipt-bound snapshot flag dla handlera/oracle. Każdy pickup `null→wartość` przechodzi przez authority, cold start inicjalizuje pełny `NEW_ORDER` przed assignmentem, a nowy rekord z rzeczywistym `None` może w jednym ticku przyjąć pierwszy pełny tuple proof-bound; malformed niepusty baseline nadal failuje closed. Przy OFF resolver jawnie zwraca ownership istniejącemu raw `first_acceptance`, więc flaga zachowuje exact legacy; przy ON ten sam response jest kanoniczny. Review v17 wykryły cztery klasy luk: unfinished authority pominięte w gate, poison re-click, zdublowaną klasyfikację i brak pierwszego null baseline. V18 ma 6F+1P przed/7P po, sześć mutation probes po 1F, focused 376/376 i pełną regresję `6599P/0F/74S/8X/149W` w 452,70 s na aktualnym masterze `49aed3215`. `forward-status` schema v3 po quiesce wymaga OFF, pustej kolejki, zera całej klasy unfinished authority/raw CK, zera pre-v4 raw eventów, zera pre-v16 assignmentów bez snapshotów i zera aktywnych niepełnych kontraktów rozpoznanych wspólnym `common.is_czasowka_order`. Dwa świeże `CLEAN` v18 nadal są wymagane przed live. Rollback hot=`false`; code revert tylko przez narzędzie po pełnym fence/backup/outbox/state preflight. Pełny kontrakt: `docs/RUTCOM_COMMITTED_PICKUP_AUTHORITY.md`.
