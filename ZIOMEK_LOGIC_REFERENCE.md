...
- `ENABLE_PLAN_CORRUPT_RAISE` — **A-2 iter3 03.08, KANDYDAT SOURCE-ONLY, default OFF, bez flipa/deployu/restartu.** ON: `plan_manager` czyta `courier_plans.json` przez jednego ownera `state_persistence`; `.prev` jest walidowanym poprzednikiem, którego brak/uszkodzony main nie nadpisuje. Normalny reader wykrywający recovery zwalnia `LOCK_SH`, bierze `LOCK_EX` i czyta ponownie. Pod EX fsyncuje cały deterministyczny zakres tokenów `>HWM`, utrwala recovered main i dopiero potem zwraca widok — token nie może zostać ponownie wydany po crashu/innym recovery. Przejściowe `EMFILE/ENFILE/ENOMEM/EIO/EINTR/EAGAIN/EBUSY/ESTALE` są ponawiane z backoffem; jawny strict caller nadal podnosi pierwotny błąd. Epoka mieści się w IEEE-754 safe integer. `world_record` przy ON snapshotuje main+HWM spójnie; replay materializuje scratchową parę przez tego samego ownera. OFF jest pełnym legacy także po wcześniejszym ON: nie czyta ani nie zapisuje `.prev`/HWM, strict missing zwraca `{}`, a wersja to `current+1`. Rollback = `flags.json false`/brak klucza hot; blind i flip należą do CTO/ownera.
- `ENABLE_ORDERS_STATE_PERSISTENCE_V2` — **A-2 iter3, default OFF, osobna bramka dla `orders_state`.** Jeden chokepoint `state_machine._write_state` obejmuje wszystkie RMW oraz bulk-prune. OFF deleguje do jednego ownera dokładny historyczny writer/read: format bajtów z `indent=2`, best-effort `.prev`, zapis mimo uszkodzonego maina/awarii backupu i dowolny JSON root. ON deleguje hardened object-only read oraz walidowany fail-closed predecessor/write. Pozostałe po ON sidecary są dla OFF inert; rollback przywraca byte-parity bez kasowania plików. Flip wymaga osobnego ACK ownera.
