...
| `ENABLE_REASSIGN_OLD_PLAN_RELEASE` | panel_watcher:`_release_plan_on_reassign` (branch reassign + PANEL_PACKS FALLBACK) | po przerzuceniu zlecenia zwalnia plan STAREGO kuriera: `remove_stops` (bump plan_version → apka starego od razu przestaje pokazywać zabrane zlecenie, zamiast czekać na fallback 180 s / 5-min plan_recheck) + recanon reszty worka (reason=`reassign_out`); lustrzane do `_remove_stops_on_return`, best-effort; default OFF, flip za ACK |
