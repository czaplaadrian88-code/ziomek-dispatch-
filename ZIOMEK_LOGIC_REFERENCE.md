...
- `ENABLE_PLAN_CORRUPT_RAISE` — **A-2 rework 03.08, KANDYDAT SOURCE-ONLY, default OFF, bez flipa/deployu/restartu.** ON: `plan_manager` czyta `courier_plans.json` przez jednego ownera `state_persistence`; `.prev` jest walidowanym poprzednikiem tworzonym przed mainem, brak/uszkodzony main nigdy go nie nadpisuje. Normalny guarded read może odzyskać poprzednika, lecz jawny strict caller zawsze podnosi pierwotny błąd przed recovery. Globalny high-water mark wersji jest fsyncowany przed mainem; recovery nadaje deterministyczne wersje `>HWM`, więc żaden stary token CAS nie jest ponownie używalny (ABA). Epoka mieści się w IEEE-754 safe integer dla konsumentów JSON. `world_record` przy ON snapshotuje main+HWM pod jednym shared lockiem, przy OFF zachowuje historyczny lockless read bez HWM; replay materializuje scratchową parę przez tego samego ownera. Epokowy snapshot bez HWM jest fail-closed. OFF na świeżym legacy store = stare zachowanie i zero nowych `.prev`/HWM oraz dokładne legacy wyjątki; po utworzeniu epoki HWM pozostaje bezwarunkowym monotonicznym fence'em także przy rollbacku flagi. Rollback zachowania = `flags.json false`/brak klucza hot; blind i flip należą do CTO/ownera.
