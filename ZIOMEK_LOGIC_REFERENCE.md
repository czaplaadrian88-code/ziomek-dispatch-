...
| `ENABLE_UWAGI_BRIDGE_NADAWCA` | panel_watcher (call-site parsera) → `uwagi_address_parser.parse_pickup_from_uwagi(bridge_format=)` | P0 bridge-NADAWCA (21.07): adres ODBIORU paczek mostu epaki (catch-all rid=161, `verbose_uwagi`) wyłącznie z terminalną kopertą `SRC:EPAKA_BRIDGE:v1`, jednym `NADAWCA:`, późniejszym jednym `Odbiorca:` i jedną kotwicą `dd-ddd`; nieznana wersja/niejednoznaczność fail-closed, extra tylko jawny kwalifikator lokalu zachowany w `number`; city z kodu → city-aware geocode. `bridge_format=True` tylko gdy ta flaga ORAZ efektywne `ENABLE_FIRMOWE_REJECT_ON_GEOCODE_FAIL` są ON (inaczej warning + legacy); bridge geocode-fail → `pickup_coords=None`/KOORD, nigdy `FIRMOWE_KONTO_FALLBACK_COORDS`; default OFF, flip za ACK Adriana |
| `ENABLE_UWAGI_BRIDGE_NADAWCA` | panel_watcher (call-site parsera) → `uwagi_address_parser.parse_pickup_from_uwagi(bridge_format=)` | P0 bridge-NADAWCA (21.07): adres ODBIORU paczek mostu epaki (catch-all rid=161, `verbose_uwagi`) z segmentu `NADAWCA:` (kotwica = token przed kodem `dd-ddd`); `oryg. adres` = DORĘCZENIE, nie odbiór; pickup_rules mostu ustawiają tylko CZAS; city z kodu pocztowego → city-aware geocode; OFF = legacy P3→KOORD (bajt-parytet); default OFF, flip za ACK Adriana |
