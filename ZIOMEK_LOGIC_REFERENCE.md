...
> | `ENABLE_CZASOWKA_RUTCOM_FORWARD_AUTHORITY` | — | **KANDYDAT v21/OFF** (02.08, incydent 491578; owner ACK na ON po wdrożeniu) | czasówka: bezpieczny ruch Rutcom CK do przodu przy aktywnym `planned`/`assigned` i `status_id=2` staje się kanonicznym `PICKUP_TIME_UPDATED`; wspólny próg 3 min, queue-bound receipt v5, immutable claim/successor/replay, courier/assignment oraz pickup+CK revision CAS, sprzężone pola i exact attestation. V21 zapisuje initial tuple jako durable intent przy `NEW_ORDER` shellu, ale jego authority wiąże exact event-id z niezależnym applied outboxem pierwotnego `NEW_ORDER`; self-hash nie wystarcza. Recovery wykonuje się przed board/fetch, raw broadcast pozostaje źródłowy, sibling writer nie może zużyć intentu, a state atomowo materializuje pickup+CK+HH:MM+provenance. Oba classifiery delegują klasę czasówki, code revert rezerwuje receipt-bound `NEW_ORDER`, a `forward-status --quiesced` schema v4 mechanicznie wymaga loaded+inactive panel-watcher i shadow. Review v20 dały 8/8 RED; v21 ma osiem mutation kills, 8/8 restore, 367/367 focused i pełną regresję `6706P/0F/74S/8X/153W` w 479,28 s. Dwa final-byte `CLEAN` pozostają wymagane; ostatni live preflight miał dwie aktywne legacy czasówki, więc flip pozostaje HOLD do ich terminalizacji i ponownego zielonego statusu |
