...
> | `ENABLE_CZASOWKA_RUTCOM_FORWARD_AUTHORITY` | — | **KANDYDAT v19/OFF** (02.08, incydent 491578; owner ACK na ON po wdrożeniu) | czasówka: bezpieczny ruch Rutcom CK do przodu przy aktywnym `planned`/`assigned` i `status_id=2` staje się kanonicznym `PICKUP_TIME_UPDATED`; wspólny próg 3 min, queue-bound receipt v5, immutable claim/successor/replay, courier/assignment oraz pickup+CK revision CAS, sprzężone pola i exact attestation. V19 wiąże initial `NEW_ORDER` z durable snapshotem: ON zapisuje shell bez raw czasu i prowadzi tuple przez jednego resolvera/writera, OFF zachowuje exact legacy, a oba detektory recovery działają przy ON niezależnie od starych kill-switchy. Complete-contract validator blokuje split/malformed state, osobny gate starego czasowego `NEW_ORDER`, contextual forward classifier zwalnia wyłącznie związany poprawny raw CK jawnego elastyka, zaś code revert nadal blokuje każdy raw CK. Wersjonowany CK/pickup po prune kończy się `superseded`. Siedem mutation kills, 432/432 focused i `6610P/0F/74S/8X/149W`; preflight 16:31Z ma dwie aktywne legacy czasówki, więc flip pozostaje HOLD do ich terminalizacji i ponownego quiesced statusu |
...
- `ENABLE_CZASOWKA_RUTCOM_FORWARD_AUTHORITY` — **KANDYDAT v19, default OFF; owner 01.08 wydał ACK na docelowe ON i wymagane kontrolowane restarty po wdrożeniu kodu**. Root cause incydentu 491578: Rutcom podał 19:21, lecz guard potraktował CK bez markera jak statusowy re-stamp i 52 razy stłumił 19:16→19:21; aplikacja poprawnie wyświetlała stary kanon. `committed_pickup_authority.py` jest jednym policy/CAS ownerem obu pól, `committed_pickup_apply.py` jednym transportem, a `PICKUP_TIME_UPDATED` jedynym writerem atomowego pickup+CK+HH:MM+provenance. Forward wymaga aktywnej czasówki, statusu Rutcom 2, ruchu ≥3 min do przodu i w przyszłość, flagi oraz passive guarda; proof wiąże klasę, courier/assignment, pickup+CK revisions i wszystkie sprzężone pola, a kolejka v5 zachowuje exact claim/successor/poison. V19 wiąże initial `NEW_ORDER` z durable snapshotem: ON tworzy aggregate shell bez raw czasu i prowadzi pełny tuple przez kanoniczny resolver, OFF zachowuje exact legacy, a forward flag wymusza oba detektory recovery po crashu. Jeden complete-contract validator blokuje split/malformed state; osobny gate starego czasowego `NEW_ORDER` i kontekstowy forward classifier zwalniają tylko poprawny, związany raw CK jawnego elastyka, podczas gdy code revert nadal blokuje każdy raw CK. Wersjonowany CK/pickup po prune kończy się `superseded`. Review v18 wykryły te luki przed live; v19 ma siedem mutation kills, 30/30 exact, 432/432 focused i pełną regresję `6610P/0F/74S/8X/149W` w 466,95 s na base `49aed3215`. `forward-status` 16:31Z ma pustą kolejkę i outbox, lecz dwie aktywne legacy czasówki z rozbieżnym pickup/CK, więc flip pozostaje HOLD do ich naturalnej terminalizacji, quiesce i ponownego zielonego preflightu. Dwa świeże `CLEAN` v19 są wymagane przed live. Rollback hot=`false`; code revert tylko przez narzędzie po pełnym fence/backup/outbox/state preflight. Pełny kontrakt: `docs/RUTCOM_COMMITTED_PICKUP_AUTHORITY.md`.
