...
> COM-P0-PERSIST-HOLD-02 v1.0 HOLD_SCOPE_MISMATCH/ZERO MUTACJI; v1.1
> HOLD_CONCURRENT_APPLY — NON-MAIN ZASTOSOWAŁ MASKĘ ZE STAREGO, ODWOŁANEGO ACK;
> POSTIMAGE FAIL-CLOSED, PROCEDURA NEEDS_RECONCILIATION
...
|---|---|---|---|---:|---|---|
| COM-P0-CONTAIN-01 | OpenClaw: trwałe containment i bezpieczne recovery po V8 OOM | Zatwierdzony recreate z loopback override wszedł w V8 heap OOM. `openclaw.service` (`Restart=always`) uruchamiał wyłącznie base Compose, odtwarzając `--bind lan` i publiczne porty. Po `HOLD_OFFLINE` stan fail-closed potwierdzono; późniejsza PERSIST-HOLD v1.1 założyła trwałą maskę, ale z naruszeniem aktualnej bramki CTO. | Osobna faza ustali OOM offline, zwiąże systemd z base+containment override, udowodni trwałość i dopiero potem wykona kontrolowany loopback-only health start bez kanałów zewnętrznych. | M | `HOLD_OFFLINE`; zachować obecną maskę. Bez startu/deployu/unmask/rollbacku/re-enable/NODE_OPTIONS. Nowe exact hashe, walidacja i osobny `ACK_RECOVERY`. | PARTIAL / FAIL_CLOSED; NIE DONE. Postcheck 15:41Z: service masked+inactive, gateway+CLI exited143, 18789/18790 absent, TG/WA false. Proceduralny status maski: `HOLD_CONCURRENT_APPLY/NEEDS_RECONCILIATION`. Raport: `eod_drafts/2026-07-15/COM_P0_CONTAIN_01_FINAL.md` |
| COM-P0-PERSIST-HOLD-02 | OpenClaw: trwała maska fail-closed bez uruchamiania usługi | v1.0 zakładała `disable → mask`, ale lokalny regularny unit blokuje `mask`. CTO odwołał wstępny ACK v1.1 jako `HOLD_ROLLBACK_FAIL_OPEN`; non-MAIN mimo tego zastosował ją ze starego, skompaktowanego kontekstu. | Zachować obecny bezpieczniejszy postimage bez dalszej mutacji: unit jest maską `/dev/null`, enablement usunięte, service inactive, 18789/18790 absent, kanały false. Następna faza naprawi bramkę/recovery; nie cofać maski w ciemno. | S | `HOLD_CONCURRENT_APPLY`/`NEEDS_RECONCILIATION`; zakaz rollbacku/startu/restartu. Nowa bramka musi mieć jednorazowy świeży gate token, immutable runbook SHA, rollback zależny od safe-state i exact gateway+CLI scope; browser/:9222 N-D. | v1.0 `HOLD_SCOPE_MISMATCH`. v1.1 zastosowana przez non-MAIN o 15:32:58Z po odwołaniu ACK; postcheck 15:41Z: masked+inactive, gateway+CLI exited, watchdog no-op, siblings healthy. Backup preimage SHA `b85e073d…07e8d` istnieje; raport wykonawcy ma status `NEEDS_RECONCILIATION`. Raport kanoniczny: `eod_drafts/2026-07-15/COM_P0_PERSIST_HOLD_02_HOLD.md` |
