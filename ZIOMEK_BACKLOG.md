...
> COM-P0-PERSIST-HOLD-02 v1.0 HOLD_SCOPE_MISMATCH/ZERO MUTACJI; v1.1 MA
> OWNER GATE, ALE CTO HOLD_ROLLBACK_FAIL_OPEN; ZERO APPLY
...
| COM-P0-PERSIST-HOLD-02 | OpenClaw: trwała maska fail-closed bez uruchamiania usługi | v1.0 zakładała `disable → mask`, ale lokalny regularny `/etc/systemd/system/openclaw.service` blokuje `mask`. v1.1 naprawiła atomic replace, lecz nakazała blanket restore do `enabled` przy każdym failed postcondition; to osłabia containment, gdy drift oznacza active/listener/channel=true. Ogólne „containers stopped” koliduje też z działającym browserem/:9222 poza zakresem. | Poprawiona faza zachowa dokładny unit, atomowo założy trwałą maskę i użyje rozgałęzionego rollbacku: restore+enable tylko przy nadal pełnym fail-closed; przy containment drift zachowa maskę i przejdzie HOLD. | S | Immutable runbook/manifest SHA + nowy exact owner gate; fresh lock/preflight; durable backup/fsync; gateway+CLI exact scope, browser/:9222 N-D; expected masked stdout/nonzero rc; naturalny watchdog tick; contingency bez nieautoryzowanego stopu. | v1.0 `HOLD_SCOPE_MISMATCH`; v1.1 `HOLD_ROLLBACK_FAIL_OPEN`; zero production mutations w obu. Unit SHA `b85e073d…07e8d`, loaded+enabled/inactive, porty absent, TG/WA false. Raport: `eod_drafts/2026-07-15/COM_P0_PERSIST_HOLD_02_HOLD.md` |
