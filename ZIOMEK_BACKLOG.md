...
> OWNER GATE + CTO ACK_PERSIST_HOLD_V1_1, CZEKA NA BOUNDED APPLY
...
| COM-P0-PERSIST-HOLD-02 | OpenClaw: trwała maska fail-closed bez uruchamiania usługi | v1.0 zakładała `disable → mask`, ale lokalny regularny `/etc/systemd/system/openclaw.service` blokuje `mask` (`file already exists`, także `--force`). Pierwszy krok zostawiłby tylko `disabled`, z nadal możliwym ręcznym startem. | v1.1 zachowa dokładny unit i metadane, usunie enablement, atomowo zastąpi ścieżkę maską `/dev/null`, wykona daemon-reload bez startu i zapewni pełny automatyczny restore przy każdym failu. | S | Exact owner scope v1.1/production przyjęty; CTO `ACK_PERSIST_HOLD_V1_1`. Apply wyłącznie z fresh preflight, single-executor lock, same-FS atomic rename i pre-authorized full restore; każdy start nadal osobną bramką. | v1.0 `HOLD_SCOPE_MISMATCH`, zero mutations. v1.1 `ACK_READY`, jeszcze NOT APPLIED przy wydaniu gate. Unit SHA `b85e073d…07e8d`, loaded+enabled/inactive, porty absent, kanały false. Raport: `eod_drafts/2026-07-15/COM_P0_PERSIST_HOLD_02_HOLD.md` |
