...
def test_blind_review_bundle_is_digest_bound_and_fail_closed():
    ...
    assert "sys.dont_write_bytecode = True" in driver
    assert 'f"  python3 -B {Path(__file__).resolve()} verify {manifest}\\n"' in driver
    ...
    assert "self-verify: bundled driver nie mutuje exact set" in selftest
    assert "self-verify: brak __pycache__ w bundlu" in selftest
