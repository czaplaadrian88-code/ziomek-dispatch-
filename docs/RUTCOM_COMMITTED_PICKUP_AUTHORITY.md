...
Status: kandydat v26 niewdrożony, kod ciemny, nowa flaga domyślnie `OFF`.
...
future/corrupt receipt zostaje fail-closed jako dowód operatorski. Rollback do
czytnika pre-v4 robi exact backup i projektuje stare, ale gotowe trwałe rekordy
na czas migracji, żeby scalar TTL nie skasował pracy; rekord przyszły blokuje
rollback zamiast zostać sztucznie przyspieszony.
...
| `coordinator_time_recheck.py` | trwała kolejka intencji | writer/reader receiptu | TAK | V6 rozdziela immutable audit `requested_at` od epoki wykonania `eligible_at` i wiąże jeden exact click-time policy lease; successor tego samego kliknięcia zachowuje lease, a nowy re-click dostaje nowy. Czytnik zachowuje v4/v5 wyłącznie jako pre-policy legacy bez prawa nabycia authority. V26 ustanawia exact queue membership jako trwały lease od enqueue do claim/exact ACK: wiek nie kasuje v4/v5/v6, future/corrupt zostaje fail-closed, a TTL dotyczy tylko starego scalaru bez `request_id`. Rollback pre-v4 rebazuje gotową trwałą pracę na czas migracji po exact backupie i blokuje future receipt. Immutable claimed head, exact-event claim, bounded continuation, claim-aware drain, poison/corrupt retention i rollback fence pozostają wspólne. V23 dodał crash-safe `forward_fence.v1`: UUID i SHA dokładnego snapshotu kolejki są tworzone pod wspólnym flockiem, a enqueue/upgrade/claim/pending-cleanup/ACK nie mutują kolejki do exact release. V24 blokuje code revert przy każdym policy-bound v6. Nie ma drugiego writera ani in-memory locka. |
...
| `tools/rutcom_committed_authority_rollback.py` | rollback/roll-forward | operator gate | TAK | Wymaga OFF każdej authority flag, terminalnego outboxa, bezpiecznej kolejki i mechanicznego quiesce exact writerów. V24 dopuszcza w forward preflight wyłącznie exact poprawny v6 unclaimed elastyk, którego receipt sam dowodzi click-time forward `OFF`; każdy v4/v5 lub malformed snapshot blokuje. `safe_for_forward_deploy` wymaga ważnego forward fence. `fence-forward` reprobuje writerów przed i po założeniu fence; `release-forward-fence` wiąże exact ID z efektywnym ON albo jawnym abortem OFF. Code revert wymaga braku forward fence i blokuje każdy policy-bound v6. |
...
| testy incident/ratchet/twins/rollback | bramka regresji | verifier | TAK | V26 odtwarza brak claimu i read-error dla obu raw typów, lifecycle po pickupie w obu stanach flagi, trwałość opóźnionego receiptu, exact ACK, rollback starej trwałej pracy i future blocker. Focused 15/15, broad 257/257, sześć mutation kills i pełna regresja 6773 passed / 74 skipped / 8 xfailed / 0 failed. Dwa świeże final-byte review v26 pozostają bramką przed live. |
