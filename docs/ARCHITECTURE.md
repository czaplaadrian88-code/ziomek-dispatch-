...
| `orders_state.json` | domenowo `state_machine`; persistence `state_persistence` | konsola, apka, silnik | prawda o zleceniach; strict RMW + predecessor `.prev` |
| `courier_plans.json` | domenowo `plan_manager` (callery m.in. `plan_recheck`); persistence `state_persistence` | konsola, apka, silnik | kanon kolejności/planów; guard ON: predecessor recovery + version-HWM anti-ABA |
