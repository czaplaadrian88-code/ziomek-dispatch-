...
| **N2** | `/root/CLAUDE.md:10` + `MEMORY.md:1` + `ZIOMEK_REGULY_KANON.md`(opis): „stan flag = DROP-INY systemd, **NIE flags.json**" | Po D3 02.07 silnik = flags.json (dowód tracker `ZIOMEK_STAN_AUDYTY_1i2.md:19` „17 kluczy=true w flags.json"). Prawda tylko dla panelu/apki. `dispatch_v2/CLAUDE.md:13` mówi ODWROTNIE („flags.json"). | SPRZECZNOŚĆ DOC↔DOC + NIEAKTUALNE | **P2** |
...
| **N4** | `ZIOMEK_DEFINITION_OF_DONE.md:10` „baseline **3611 passed / 2 failed**" | Bieżąca regresja 4096–4110/0-1 (tracker l.10-11: „4110/0", „4101/1-flaky"). Baseline ~500 testów za mały. | NIEAKTUALNE | **P3** |
| **N5** | `workspace/CLAUDE.md:5,113,118` „Adrian does NOT decide / does NOT make technical decisions / NOT ask clarifying questions" + model AIDER/DeepSeek | Sprzeczne z `/root/CLAUDE.md` Przykazanie #0 („Wątpliwość → **PYTAJ Adriana**", ACK dla ryzyk) + `feedback_rules.md`. Auto-ładowany w hierarchii dispatch_v2. Dokument-widmo wczesnej fazy. | SPRZECZNOŚĆ DOC↔DOC | **P1** |
...
| **N12** | `/root/CLAUDE.md:111` „flag `ENABLE_COMMIT_DIVERGENCE_VERDICT_GATE` **default ON**" | `flags.json=false` → efektywnie **OFF** („mina rozbrojona" w `ziomek-fala-l0`, lookup nie zaktualizowany). | SPRZECZNOŚĆ DOC↔KOD | **P2** |

**Podsumowanie ilościowe:** 12 niezgodności. Wg typu: DOC↔DOC = 3 (N2,N3,N5), DOC↔KOD = 5 (N1,N8,N11,N12,+N2 częściowo), NIEAKTUALNE = 4 (N2,N4,N6,N7), BRAK-W-DOC = 2 (N9,N10). Wg wagi: **P1 = 1** (N5), **P2 = 7** (N1,N2,N3,N8,N9,N11,N12), **P3 = 4** (N4,N6,N7,N10). Zero P0 w warstwie DOC-consistency (P0-security = inny agent, §8).
...
6. **`workspace/CLAUDE.md`:** uznaję za **NIEobowiązujący** (sprzeczny z całym bieżącym modelem pracy) — do usunięcia/przepisania decyzją Adriana.
...
4. **N5/`workspace/CLAUDE.md`:** czy skasować/przepisać ten plik? Jest w hierarchii auto-load i wprost każe „nie pytać Adriana" — ryzyko, że inna sesja go potraktuje jako regułę.
5. **P0-SECURITY** (firewall hosta, `/stop` bez auth :8765, CDP :9222 — z `ziomek-audyt-2-wyniki-2026-07-02`): udokumentowane i śledzone, ale **weryfikacja portów należy do innego agenta** — świadomie NIE sondowałem (poza rolą DOC-consistency + potencjalnie inwazyjne).
6. **Konwencja ścieżek w docs:** relatywne `dispatch_state/…`, `tools/…` bez kotwicy repo dają fałszywe „MISS" (§4). Do rozważenia: kotwiczyć ścieżki od `workspace/` w lookup.
