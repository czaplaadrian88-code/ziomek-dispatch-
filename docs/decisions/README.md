...
**Uwaga:** ADR-y opisują STAN OBOWIĄZUJĄCY, ale nie zastępują Przykazania #0 (`memory/ziomek-change-protocol.md` — JAK bezpiecznie zmieniać) ani kanonu (`ZIOMEK_ARCHITECTURE.md`/`ZIOMEK_INVARIANTS.md`). Jawny Owner Decision Record ma pierwszeństwo w semantyce produktu; kod i runtime nie mogą go nadpisać.
...
| [ADR-003](ADR-003-always-propose.md) | Always-propose: przy flocie jawny feasible albo `NO/ALERT` least-damage; widoczność problemu nie nadaje feasible/execute, a trudne klasy początkowo wymagają approval. |
