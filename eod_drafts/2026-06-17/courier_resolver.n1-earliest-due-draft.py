...
def _earliest_due_pickup_key(o: dict) -> tuple:
    """N1 (2026-06-17): klucz do wyboru NAJWCZEŚNIEJ NALEŻNEGO odbioru.

    Kurier bez GPS z workiem samych przypisanych — kotwiczymy pozycję do odbioru
    z najwcześniejszym umówionym czasem (czas_kuriera_warsaw → pickup_ready_at),
    bo tam realnie zmierza ("weź czas w restauracji za pewnik"), a NIE do
    najnowszo-przypisanego, który mógł być na drugim końcu miasta.

    min() po tym kluczu: (0, najwcześniejszy_czas) < (1, _) → brak czasu na koniec.
    """
    ts = (parse_panel_timestamp(o.get("czas_kuriera_warsaw"))
          or parse_panel_timestamp(o.get("pickup_ready_at")))
    if ts is None:
        return (1, DT_MIN_UTC)
    return (0, ts)
        ...
        if active_bag_orders:
            ...
            for order in sorted_bag:
                if order.get("status") == "picked_up":
                    ...
                    # N1 (2026-06-17, flaga ENABLE_GPS_LESS_EARLIEST_DUE_ANCHOR):
                    # kurier bez GPS, worek same przypisane → kotwicz pozycję do
                    # NAJWCZEŚNIEJ NALEŻNEGO odbioru (czas_kuriera), NIE do
                    # najnowszo-przypisanego (mógł być daleko: 413→Dentomax daleki E
                    # zamiast Rukola należnej 13:07 → fałszywe +10-15min w planie).
                    # Flaga OFF → _anchor = order = zachowanie sprzed N1.
                    _anchor = order
                    if flag("ENABLE_GPS_LESS_EARLIEST_DUE_ANCHOR", False):
                        _assigned_cands = [
                            _o for _o in active_bag_orders
                            if _o.get("status") != "picked_up" and _o.get("pickup_coords")
                        ]
                        if _assigned_cands:
                            _anchor = min(_assigned_cands, key=_earliest_due_pickup_key)
                    if _anchor.get("pickup_coords"):
                        cs.pos = tuple(_anchor["pickup_coords"])
                        cs.pos_source = "last_assigned_pickup"
                        resolved = True
                        break
                    _log.warning(
