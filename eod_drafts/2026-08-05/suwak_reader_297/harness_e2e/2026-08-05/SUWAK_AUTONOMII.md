...
**Wygenerowano:** 2026-08-05T08:13:36Z · **Tryb:** READ-ONLY (zero zapisu do stanu, flag, telegramu, restartów)
**Czytelnik:** `dispatch_v2/tools/suwak_autonomii_review.py` (dobowy, pod `shadow-review.timer`)
**Szereg czasowy:** `/root/worktrees/dispatch_v2/active/20260805-suwak-reader-297-cto/eod_drafts/2026-08-05/suwak_reader_297/harness_e2e/suwak_autonomii.jsonl`

---

## ⭐ ODPOWIEDŹ W DWÓCH LICZBACH

**LICZBA 1 — ile Ziomek zrobiłby SAM:** **19.72%** decyzji kwalifikuje się do auto-assign w wariancie D (n=1552, okno 2026-07-28..2026-08-05, 9 dni).
Dla porównania: bazowy (dzisiejsze bramki) **1.03%**, wariant D' **17.59%**, kanał `auto_route=AUTO` **10.05%**.
...
| Miara | % | true / n |
|---|---|---|
| bazowy `would_auto_assign` (dzisiejsze bramki) | 1.03% | 16 / 1552 |
| **wariant D `would_auto_assign_d`** | 19.72% | 306 / 1552 |
| wariant D' `would_auto_assign_dprime` | 17.59% | 273 / 1552 |
| kanał `auto_route == AUTO` | 10.05% | 156 / 1552 |

Rozkład kanału `auto_route`: `{'ACK': 725, 'ALERT': 671, 'AUTO': 156}`
...
| pool<=2 | 478 | 0.00% | 6.90% | 0.00% | 3.56% |
| nieznana | 0 | n/d | n/d | n/d | n/d |

### Co blokuje pozostałe 1246 decyzji (wariant D)

Rekordy z zapisanym powodem: 1147 / 1246. Rodziny powodów (rekord może mieć kilka; liczone unikalnie na rekord):
...
Wspólnych zamówień w oknie nakładania: **1526** (z 1545 outcomes w oknie i 1552 zindeksowanych decyzji).
...
Join po `order_id`; zamówienie może mieć kilka decyzji (przekierowania) — brana jest **pierwsza** decyzja dyspozytorska. Zamówień z >1 decyzją: 0 z 1552 (max 1 decyzji na zamówienie).
...
4. **Wykluczenia z mianownika Liczby 1:** 60 rekordów lifecycle (`CZASOWKA_RECLAIM_EVALUATION`), 0 bez pól bramki auto, 0 duplikatów po `event_id`, 0 nieparsowalnych linii.
...
6. **Żywy plik.** `shadow_decisions.jsonl` jest dopisywany na bieżąco; SHA-256 w manifeście opisuje stan z momentu odczytu (2026-08-05T08:13:35Z).

## MANIFEST WEJŚĆ

| Plik | rekordy | złe linie | okno | rozmiar | mtime (UTC) | SHA-256 |
|---|---|---|---|---|---|---|
| `/root/.openclaw/workspace/scripts/logs/shadow_decisions.jsonl` | 384 | 0 | 2026-08-03..2026-08-05 | 36943685 B | 2026-08-05T08:10:36Z | `9d9b7e305a10bd12f2822d2055e989ab3c5eb6a029b4dae3ca910d9b24739087` |
...
Start odczytu: 2026-08-05T08:13:35Z · koniec: 2026-08-05T08:13:36Z
