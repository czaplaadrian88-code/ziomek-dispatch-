...
## 20. CZASÓWKA INTERMITTENT-COLD = REFUTED (kłamiący przyrząd) + fix log-hygiene (tmux 14, 2026-07-03 ~06:15 UTC; append-only)
Eskalacja P1 z §19 ZAMKNIĘTA — **silnik czasówki ZDROWY**: korelacja `logs/czasowka.log`↔journalctl (okno 00:00-05:49, 695 linii FLAG_FINGERPRINT, 334 starty serwisu): **334/334 ticków WARM (flagi=flags.json), 0/9 klastrów cold pochodzi od serwisu** (wszystkie 3-linijkowe, poza kadencją ticków; 8× nocna regresja pytest 01:27-01:56 + 1× 05:36:33 bez startu serwisu w journalu). „22-40%" = udział TESTOWYCH linii w oknie pomiaru, nie flag-load silnika. **6. przypadek C9** — przyrząd (fingerprint-check) czytał PROD-log, do którego pisały testy pytest z conftest-owo ODARTYM flags.json (defaulty), przez module-level `setup_logger(..., PROD path)` (~34 moduły, FileHandler przy imporcie).
- **Fix u źródła (1 miejsce → 34 moduły):** `common.setup_logger` — `_ProdFileLogTestFilter` per-rekord na FileHandlerze (markery `PYTEST_CURRENT_TEST`/`DISPATCH_UNDER_PYTEST`, opt-out `ALLOW_FILE_LOG_IN_TEST=1`, wzorzec = guard telegram L1) + `delay=True`; `tests/conftest.py` marker sesji; opis findingu w toolu uzupełniony o obowiązkową korelację z journalem.
- **Dowody:** 5 nowych testów + mutacja M1 RED→GREEN; e2e: bieg polluterów (27 testów main()) → PROD-log 707→707, 0 nowych cold; tool po fixie: INTERMITTENT-COLD zniknął; pełna regresja vs baseline w commicie. PROD nietknięty (bez markerów env FileHandler pisze jak dotąd) — zero restartów.
- **Protokół żywy:** wzorzec #10 dopisany (werdykt z loga wymaga korelacji z journalem; mina „testy piszą do PROD" obejmuje LOGI). Raport: `eod_drafts/2026-07-03/czasowka_intermittent_cold_raport.md`.
- ⭐ Obserwacja poza pasem: 2. okaz flaky script_run pod pełną suitą (`test_bug1_invalidate_plan_on_bag_change_2026_06_05`, 3/3 pass izolowany) — ta sama rodzina co `test_v319c_sub_a` (zad. 3 handoffu).

## 21. FLAKY v319c = REGRES FLIPU PERF_LAZY (nie pre-existing) — mtime-cache planów naprawiony write-through+st_ino (tmux 14, 2026-07-03 ~07:00 UTC; append-only)
Zad. 3 handoffu domknięte. **Root cause:** założenie cache'a planów „os.replace bumpuje mtime" FAŁSZYWE — zapisy w tym samym ticku zegara jądra = identyczny `st_mtime_ns` (+równy size przy ciałach tej samej długości) → czytelnik dostawał stan sprzed zapisu. Flaka urodziła się z flipem `ENABLE_PERF_LAZY_MEMBERS` 00:25 (script-runnery dziedziczyły żywy flip — flaga nie była na listach strip). **Dowód ON≠OFF:** przed fixem ON=4/30 FAIL / OFF=0/30; po fixie ON=0/30 + 3×izolowany PASS.
- **Fix (3 warstwy):** `_write_raw` write-through czyści cache · klucz `(mtime_ns,size,st_ino)` (os.replace=nowy inode → cross-proces) · `ENABLE_PERF_LAZY_MEMBERS`→`TEST_ISOLATED_INFRA_FLAGS` (determinizm suity). 6 nowych testów + mutacja M1 RED→GREEN. Dziura była też PROD-owa (write→read-back w tym samym ticku) — oneshoty łapią od ticku, shadow/pw przy najbliższym restarcie (ms-owa, bez eskalacji przed peakiem). Wpływ na pomiar perf zad.1 = brak (ciepła ścieżka odczytu identyczna).
- Raport: `eod_drafts/2026-07-03/flaky_v319c_perfcache_raport.md`. 2. okaz (`test_bug1_invalidate…`) pokryty tym samym fixem (obserwacja: wróci → osobna diagnoza).

## 22. SESJA DZIENNA 03.07 (~10:00-13:35 UTC; GO Adriana „jedziemy po kolei 1234" + ACK-i; append-only)
**Wykonane (chronologicznie, wszystko PUSHED origin/master do `a6473a4`):**
- **at-207 werdykt perf** = 🟢 POPRAWA REALNA, `ENABLE_PERF_LAZY_MEMBERS` zostaje ON (okno 00:30→11:13 n=60: p50 620/−27%, p95 1551/−20%, ogon 6,7%/−49% vs baseline 852/1939/13,1%). ⭐ Peak p95 1810≈1847 = ogon w peaku ma INNE źródło niż IO → kandydat osobnej fali perf. Doc: `2026-07-03/perf_verdict_at207.md`. **at-209 So 04.07 08:00 UTC = bramkowany flip `ENABLE_PERF_SLO_ALERT` (ACK, rekomendacja a; profil `perf-slo` w scheduled_flip_gate, dry-run DRY_OK).** 1. tick log-rotation 03:01 SUCCESS.
- **L8-iter3** (`cbe566f`): sprint2_analysis skasowany (21 plików/1670 linii); ratchet TZ nienaruszony (dowód: probe fixed-offset → RED); 3 dowody śmierci; test_common_to_warsaw usunięty jako bezprzedmiotowy.
- **fingerprint-guard timer** (`52f57d0`): strażnik flag 30-min z korelacją journalem (COLD poza oknem biegu = UNVERIFIED, anty-§20 wzorzec #10), edge-triggered, alert za flagą OFF; 11 testów + mutation ×2. ⏳ **INSTALACJA unitów zablokowana przez classifier (nowa trwała usługa) — skrypt gotowy: `dispatch_state/install_fingerprint_guard.sh` (Adrian: `! bash <ścieżka>`).**
- **frozen-objektyw P0** (`3dbaf87`, measure-first `frozen_objektyw_measure_design.md`): tripwire objektywu bug4 był MARTWY (frozen=null 128/128); wariant a-faithful — frozen scorowany PURE-helperami oracle (jedno źródło), fast-path 54,7%, marginał OSRM≈0 (cache-hit); LIVE od ticku 11:59, 1. realny pomiar: RETIME lepszy o 7,8 min od zamrożonej sekwencji.
- **L7 tanie podkroki (4 pasy równoległe, worktree)**: L7.8 (`2c5d17b`) kierunek werdyktu sequential_replay (mapa `_HIGHER_IS_BETTER`; produkcyjnie nic nie zafałszowane — offline CLI bez wołaczy) · L7.1 (`6c420c2`) tripwire R-DECLARED w `state_machine.upsert_order` — **pomiar: 11/168 zleceń ~6,5% łamie `czas_kuriera≥czas_odbioru` DZIŚ** · L7.4 (`af0c144`) `tools/feas_carry_outcome_join.py` — **⭐ 83,7% wybaczeń fizycznie NIESZKODLIWYCH (n=1318), projekcja regret przeszacowana; realne re-admity 4, 0/4 wykonane → TWARDA BRAMKA przed re-flipem #483000** · L7.3 (`0f2b04f`) split-layer guard INV-LAYER-1/2 (jedyna mutacja verdiktu poza L5 = FEAS_CARRY_READMIT z selekcji; raw-write 2→1-w-setterze) + recon free_at_min→L4 (w raporcie l73; 🔴 osobny pas).
- **DEPLOY za ACK (~13:18-13:20):** restart dispatch-shadow + dispatch-panel-watcher (oba czyste 0 ERROR) → flip hot `ENABLE_R_DECLARED_TRIPWIRE=true` + `ENABLE_SPLIT_LAYER_GUARD=true` (+ jawny `ENABLE_FLAG_FINGERPRINT_GUARD_ALERT=false`); backup `flags.json.bak-pre-l7-flips-20260703`; 3 flagi udokumentowane w LOGIC_REFERENCE (ratchet flag-doc wymusił — zadziałał).
**Regresja dnia: 4110/0 → 4165/0** (net +55 testów po −1 świadomie usuniętym). **ZOSTAJE:** L6.C (bramka at-201 dziś 19:00) · USE_V2_PARSER→ETAP4 (Fala D, po Fali C/L3) · L7.2/L7.6/L7.7/free_at_min→L4 (bramkowane) · security P0 (Adrian krok 0) · instalacja fingerprint-guard (skrypt czeka).
