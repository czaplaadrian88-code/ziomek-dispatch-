...
| **Strażnicy feasibility cienkie/teatr** (verdict-gate polaryzacja) | P2 | 🟡 **FALA-2 02.07: ODSŁONIĘTE + DOGĘSZCZONE** — mutation-probe (`tools/guard_mutation_probe.py`): 3/6 HARD-bramek teatralnych (bag-cap string-match, verdict-gate polaryzacja, próg SLA); 13 testów behawioralnych (≥2 kills/bramka) + wariant polaryzacyjny zabijają 4/5+1 mutantów. **Zostały 2 xfail L-TEATR-1/2** — korzeń: R6↔SLA maskują się na wspólnym 35min → fix U ŹRÓDŁA (SERIAL) = konsolidacja 35-min HARD z jawną kotwicą (te same 3 bliźniaki co prerekwizyt flipu O2!) |
