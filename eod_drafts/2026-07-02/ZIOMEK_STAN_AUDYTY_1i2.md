...
| **grafik UTC vs Warsaw + literówka** | P2 | 🟡 **FALA-2 02.07: ZLOKALIZOWANE precyzyjnie** — `scripts/fetch_schedule.py:130` (H1 today=UTC → okno 00:00-02:00 Warsaw ładuje WCZORAJSZY grafik) + `:121` (H2 literówka godziny → cały wpis kuriera=None → wypada z puli). **Plik POZA repo dispatch_v2 = poza ratchetem TZ (blind-spot zgłoszony), ZERO testów.** Design fixu gotowy w `tz-drobnica_raport.md` → fala SERIAL (H1 poprawność; H2 decyzyjne za flagą+ACK) |
