...
## USE_V2_PARSER
env-frozen `panel_client.py:93` default "0"; env: **tylko panel-watcher=1**. Gałąź decyzyjna = `parse_panel_html` (l.425-436) — woła ją decyzyjnie TYLKO panel-watcher; **shadow nie parsuje panelu tą ścieżką** (konsumuje stan pisany przez pw). Migracja=1 → v2 wszędzie, gdzie ktokolwiek woła parse_panel_html. **WYMAGA-DECYZJI, ryzyko średnie** (v1 ma znany latentny bug rollovera `46\d{4}`).
