...
regresja: DELTA vs baseline = 0 failed nowych i 0 zniknięć (pełna suita, harness pkgroot ZIOMEK_SCRIPTS_ROOT + -p no:cacheprovider; baseline czysty 7e57085 = 9 failed/5197 passed/27 skipped/7 xfailed, kandydat v2 FINALNY = 9 failed/5221 passed/24 skipped/7 xfailed = 5197 + 21 nowych + 3 warunkowe skipy przeszły; 9 faili = bajt-identyczny obustronny szum harnessu: script_run ×3, flag_doc_coverage ×3, conftest_flag_strip_guard ×3 — potwierdzony na czystym masterze)
e2e: zapis kanonu (plan_manager CAS) → recanon_courier/redecide_courier → pin → _retime_stops (OSRM) → ewaluacja HARD po pinie → projekcja route_order.order_podjazdy(trust_canon) = konsument konsoli+apki; testy test_pin_transparent_for_surfaces_via_route_order + test_gen_path_pins_sequence (realny OR-Tools) + test_recanon_after_raw_save_reapplies_pin (sekwencja handlera assign); nowe testy 21/21
