"""Phase 0 PoC analysis — czyta najnowszy results_*.jsonl (TomTom), summary stats per bucket."""
import glob
import json
import statistics
import sys
...
def main():
    ...
    for r in rows:
        ...
        buckets[b]["lat_tomtom"].append(r["tomtom"]["latency_ms"])
        buckets[b]["lat_osrm"].append(r["osrm"]["latency_ms"])
        buckets[b]["tt_overhead"].append(r.get("tomtom_traffic_overhead_min", 0))
    header = (
        "bucket".ljust(10) + " "
        + "n".rjust(3) + " "
        + "|d_stat| med".rjust(13) + " "
    ...
    for b in ["peak", "shoulder", "offpeak"]:
        ...
        soh = _stats(d["tt_overhead"])
        ltt = _stats(d["lat_tomtom"])
        ...
        line = (
            ...
            + str(soh.get("median", "-")).rjust(11) + " "
            + str(ltt.get("p95", "-")).rjust(11) + " "
            + str(lo.get("p95", "-")).rjust(13)
        )
        print(line)
    print()
    print("GATE A criteria: peak |d_stat| median >= 2.0 AND lat_tomtom p95 <= 400 -> PROCEED Phase 1")
