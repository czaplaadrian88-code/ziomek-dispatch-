...
---
# KOREKTY Adriana (06.07 wieczór, obowiązujące)
1. **Obietnica DYNAMICZNA = NIE** — czas jest JEDEN (kontrakt; przewaga nad Uber/Wolt). Wyjątek w planach: uprzedzenie restauracji + zmiana czasu przy spóźnieniu >10 min — dopiero PO migracji restauracji gastro→gps.nadajesz.pl.
2. **PACZKI WYKLUCZONE z kalibracji obietnicy** (szerokie okna odbioru/doręczenia): kanon = `is_paczka_order` (address_id ∈ PACZKA_ADDRESS_IDS {161,232-236}). Po wykluczeniu (2,1% rekordów): jedzeniówka med +8,2 → bufor bazowy 3,2 min; odchyleńcy stabilni 5/40 (Baanko +14, Pizzeria105 +13,5, Hacienda −0,1). ⚠ PRE-FLIP: guard paczki w `_promised_pickup_fields` (decision record MA address_id — payload:1120) — bufor tylko dla jedzeniówki.
3. **GO Adriana: SPRINT POMIAROWY dekompozycji poślizgu** („to robimy i niech liczy") → raport `SLIP_DECOMPOSITION_raport.md` (agent w tle 06.07).
